  ...
  *)
  ...
  if [ "$(uname)" = "Darwin" ]; then
    ...
    if [ "$PACKAGE_TYPE" = "dmg" ]; then
      ...
      OUT_DIR="$REPO_ROOT/result-dmg-$VARIANT"
  ...
  for TYPE in $TYPES; do
    ...
    deb)
      if [ "$(uname)" = "Linux" ]; then
        ...
        nix-shell -I "nixpkgs=${NIXPKGS_ARG}" -p curl --run "bash '$SCRIPT_LINUX' --variant '$VARIANT'"
        REAL_OUT="$REPO_ROOT/result-deb-$VARIANT"
        echo "Built deb ($VARIANT): $REAL_OUT"
      else
    ...
    rpm)
      if [ "$(uname)" = "Linux" ]; then
        ...
        nix-shell -I "nixpkgs=${NIXPKGS_ARG}" -p curl --run "bash '$SCRIPT_LINUX' --variant '$VARIANT'"
        REAL_OUT="$REPO_ROOT/result-rpm-$VARIANT"
        echo "Built rpm ($VARIANT): $REAL_OUT"
      else
      ...
      ;;
    dmg)
      ATTR="kms-server-${VARIANT}-dmg"
      OUT_LINK="$REPO_ROOT/result-dmg-$VARIANT"
      nix-build -A "$ATTR" -o "$OUT_LINK"
      REAL_OUT=$(readlink -f "$OUT_LINK" || echo "$OUT_LINK")
      echo "Built dmg ($VARIANT): $REAL_OUT"
      ;;
    *)
    ...
    if ! (
      set -euo pipefail
      echo "Running smoke test for $TYPE ($VARIANT)…"
      tmpdir=$(mktemp -d)
      ...
      echo "Smoke test FAILED for $TYPE ($VARIANT)" >&2
      exit 1
    fi
