...
echo "Purging any soft-deleted secret with the same name (idempotent)..."
PURGE_STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X DELETE \
  "${KV_BASE_URL}/deletedsecrets/${SECRET_NAME}?api-version=7.4" \
  -H "Authorization: Bearer ${KV_TOKEN}") || true
echo "Purge response: HTTP ${PURGE_STATUS} (404 means not in recycle bin — OK)"
# Azure purge is async; wait a moment for it to propagate
if [ "${PURGE_STATUS}" = "204" ]; then
  echo "Waiting 10s for purge to propagate..."
  sleep 10
fi
