...
if [ -f /etc/lsb-release ]; then
  ...
  HSM_MODEL=utimaco HSM_USER_PASSWORD="$HSM_USER_PASSWORD" HSM_SLOT_ID=0 cargo test --target "$TARGET" --features non-fips -- tests::hsm::test_all
  ...
  sudo -E env "PATH=$PATH" HSM_MODEL=softhsm2 HSM_USER_PASSWORD="$HSM_USER_PASSWORD" HSM_SLOT_ID="$HSM_SLOT_ID" cargo test --features non-fips -- tests::hsm::test_all
