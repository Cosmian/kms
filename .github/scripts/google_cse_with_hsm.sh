...
$COSMIAN sym keys import -f json-ttlv -r -t "$GOOGLE_KEY_ID" -w "$HSM_KEY_ID" "$CSE_KEY" "$GOOGLE_KEY_ID"
...
# $COSMIAN sym keys create "$GOOGLE_KEY_ID"

# Reusing the created certificate
$COSMIAN google key-pairs create \
...
ISSUER_PRIVATE_KEY_ID=$(COSMIAN_KMS_CLI_FORMAT=json $COSMIAN attributes get \
