...
setup_venv_and_pykmip() {
    ...
    cat > .github/scripts/pykmip/activate_venv.sh << 'EOF'
#!/bin/bash
...
EOF
    chmod +x .github/scripts/pykmip/activate_venv.sh
    print_status "Created .github/scripts/pykmip/activate_venv.sh for easy activation"
}
...
create_config() {
    ...
    if [[ ! -f ".github/scripts/pykmip/pykmip.conf" ]]; then
        print_status "PyKMIP configuration already exists"
    else
        print_status "PyKMIP configuration file created at .github/scripts/pykmip/pykmip.conf"
    fi
}
...
test_setup() {
    ...
    if [[ -f ".github/scripts/pykmip/pykmip_client.py" ]]; then
        if python .github/scripts/pykmip/pykmip_client.py --help &> /dev/null; then
    ...
    if [[ -f ".github/scripts/pykmip/test_pykmip.sh" ]]; then
        chmod +x .github/scripts/pykmip/test_pykmip.sh
        print_status "Test script is ready at .github/scripts/pykmip/test_pykmip.sh"
    fi
}
...
show_next_steps() {
    ...
    echo "   source .github/scripts/pykmip/activate_venv.sh"
    echo
    echo "2. Start your KMS server with socket server enabled:"
    echo "   COSMIAN_KMS_CONF=.github/scripts/pykmip/kms.toml cargo run --bin cosmian_kms "
    echo
    echo "3. Test PyKMIP connectivity (in another terminal with venv activated):"
    echo "   ./.github/scripts/pykmip/test_pykmip.sh check"
    echo
    echo "4. Run PyKMIP operations:"
    echo "   ./.github/scripts/pykmip/test_pykmip.sh query"
    echo "   ./.github/scripts/pykmip/test_pykmip.sh all"
    echo
    echo "5. Run Rust integration tests:"
    echo "   cargo test test_pykmip --package cosmian_kms_server"
    echo
    echo "6. For more details, see:"
    echo "   .github/scripts/pykmip/README_PYKMIP.md"
    echo
