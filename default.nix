...
rec {
  ...
  expected-hash-server-fips-static =
    let
      ...
      name = "server.fips.openssl.${arch}.${os}.sha256";
    in
    pkgs.runCommand "expected-hash-server-fips-static"
      ...
      ''
        set -euo pipefail
        mkdir -p "$out"
        bin='${kms-server-fips}/bin/cosmian_kms'
  ...
  expected-hash-server-fips-dynamic =
    let
      ...
      arch = builtins.elemAt parts 0;
      os = builtins.elemAt parts 1;
      name = "server.fips.no-openssl.${arch}.${os}.sha256";
    in
    pkgs.runCommand "expected-hash-server-fips-dynamic"
      ...
      ''
        set -euo pipefail
        mkdir -p "$out"
        bin='${kms-server-fips-dynamic}/bin/cosmian_kms'
        [ -x "$bin" ] || { echo "Missing binary: $bin" >&2; exit 1; }
        hash="$(${pkgs.openssl}/bin/openssl dgst -sha256 -r "$bin" | awk '{print $1}')"
        printf '%s\n' "$hash" >"$out/${name}"
      '';
  ...
  expected-hash-server-non-fips-static =
    let
      ...
      arch = builtins.elemAt parts 0;
      os = builtins.elemAt parts 1;
      name = "server.non-fips.openssl.${arch}.${os}.sha256";
    in
    pkgs.runCommand "expected-hash-server-non-fips-static"
      ...
      ''
        set -euo pipefail
        mkdir -p "$out"
        bin='${kms-server-non-fips}/bin/cosmian_kms'
        [ -x "$bin" ] || { echo "Missing binary: $bin" >&2; exit 1; }
        hash="$(${pkgs.openssl}/bin/openssl dgst -sha256 -r "$bin" | awk '{print $1}')"
        printf '%s\n' "$hash" >"$out/${name}"
      '';
  ...
  expected-hash-server-non-fips-dynamic =
    let
      ...
      arch = builtins.elemAt parts 0;
      os = builtins.elemAt parts 1;
      name = "server.non-fips.no-openssl.${arch}.${os}.sha256";
    in
    pkgs.runCommand "expected-hash-server-non-fips-dynamic"
      ...
      ''
        set -euo pipefail
        mkdir -p "$out"
        bin='${kms-server-non-fips-dynamic}/bin/cosmian_kms'
        [ -x "$bin" ] || { echo "Missing binary: $bin" >&2; exit 1; }
        hash="$(${pkgs.openssl}/bin/openssl dgst -sha256 -r "$bin" | awk '{print $1}')"
        printf '%s\n' "$hash" >"$out/${name}"
      '';
