...
/// Like `test_app_with_audit` but with `AuditFailureMode::Reject` and a
/// caller-supplied store (use `AuditFileStore::new_disconnected()` to guarantee
/// every `enqueue()` call fails without a timing race).
pub(crate) async fn test_app_with_audit_and_store(
