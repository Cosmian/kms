...
async fn test_multi_hsm_routing() -> KResult<()> {
    ...
    assert_eq!(
        create_response2.unique_identifier,
        UniqueIdentifier::TextString(key2_uid.clone())
    );

    // Clean up: destroy both HSM keys so they do not linger in SoftHSM2 slots and
    // interfere with later tests (e.g. the pkcs11-tool attribute-warning check which
    // lists all objects in slot 0 and fails on sensitive keys it did not create).
    for uid in [&key1_uid, &key2_uid] {
