...
openssl req -new -x509 -key ca.key -out ca.crt -days 2 -subj "/C=FR/ST=IdF/L=Paris/O=Cosmian/OU=R&D/CN=Test CA"
...
# certificate without the CRL.
openssl x509 -req -in intermediate.csr -CA ca.crt -CAkey ca.key -out intermediate.crt -days 1825
# certificate with CRL.
openssl x509 -req -in intermediate.csr -CA ca.crt -CAkey ca.key -out intermediate_crl.crt -days 1825 -extensions v3_ca -extfile extensions.ext
...
# certificate without the CRL.
openssl x509 -req -in leaf.csr -CA intermediate.crt -CAkey intermediate.key -out leaf.crt -days 1825
# certificate with CRL.
openssl x509 -req -in leaf.csr -CA intermediate.crt -CAkey intermediate.key -out leaf_crl.crt -days 1825 -extensions v3_ca -extfile extensions.ext
