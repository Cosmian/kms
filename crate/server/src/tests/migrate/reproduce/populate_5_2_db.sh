...
CC_OUTPUT=$($COSMIAN_CLI cc keys create-master-key-pair -s "$TEMP_JSON" -t cat -t dog)
...
$COSMIAN_CLI access-rights grant mt_owner "$OWNER_PERMISSIONS" -i "$CC_SK_ID"
$COSMIAN_CLI access-rights grant mt_owner "$OWNER_PERMISSIONS" -i "$CC_PK_ID"
$COSMIAN_CLI access-rights grant mt_owner "$OWNER_PERMISSIONS" -i mt_rsa
$COSMIAN_CLI access-rights grant mt_owner "$OWNER_PERMISSIONS" -i mt_rsa_pk
$COSMIAN_CLI access-rights grant mt_owner "$OWNER_PERMISSIONS" -i mt_normal_aes
# Grant permissions to mt_normal_user
echo "Granting permissions on mt_normal_aes to mt_normal_user"
$COSMIAN_CLI access-rights grant mt_normal_user get encrypt decrypt -i mt_normal_aes
...
# ./target/debug/ckms access-rights grant mt_owner locate -i mt_exists
