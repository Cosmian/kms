...
#[actix_rt::test]
async fn test_insert_tags() -> KResult<()> {
    let dir = tempdir()?;
    ...
    // SQL Cipher test
    {
        ...
        let file_path = dir.path().join("test_sqlite_enc.db");
        if file_path.exists() {
        ...
        insert_tags(db, Some(&params)).await?;
    }
    ...
    #[cfg_attr(feature = "sqlcipher", ignore)]
    {
        ...
        insert_tags(db, None).await?;
    }
    Ok(())
}

async fn insert_tags<DB: Database>(db: DB, db_params: Option<&ExtraDatabaseParams>) -> KResult<()> {
    log_init("info");
