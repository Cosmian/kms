...
impl CASigningKey {
    pub async fn new(ca: &str, private_key_uid: &str, public_key_uid: &str) -> KResult<Self> {
        Ok(Self {
            ca: ca.to_string(),
    ...
    pub async fn from_private_key_uid(
        ...
        Self::new(ca, private_key_uid, public_key_uid).await
    }

    pub async fn build_key_pair(
        &self,
        kms: &KMS,
        owner: &str,
        params: Option<&ExtraDatabaseParams>,
