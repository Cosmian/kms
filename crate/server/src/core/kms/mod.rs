...
impl KMS {
    ...
    ///
    /// The `PostgreSQL` backend connects, acquires its advisory lock, and verifies the
    /// entire chain SYNCHRONOUSLY here — a failure propagates up and aborts server
    /// startup. The File backend returns immediately; recovery and lock acquisition
    /// self-heal in the background — see `AuditStore::start_with_max_size`.
