...
pub struct KMS {
    ...
    /// Audit store (if audit logging is enabled)
...
impl KMS {
    ...
    /// Starts the audit store if audit logging is configured.
    async fn create_audit_store(server_params: &ServerParams) -> KResult<Option<AuditStore>> {
        if let Some(ref audit) = server_params.audit {
            let store = AuditStore::start(audit).await?;
            Ok(Some(store))
        } else {
            Ok(None)
        }
