...
pub(crate) trait RekeyOperation {
    ...
    ///
    /// Returns one or more [`RotationCandidate`]s (existing objects eligible for rotation).
    /// For symmetric keys this is 1 candidate; for key pairs, 2 (SK + PK); for certs, 1.
    fn validate(
        &self,
        kms: &KMS,
    ...
    ///
    /// Returns one [`ReplacementObject`] per new object to create.
    /// For key pairs this may return 2 objects from 2 candidates.
    fn generate_replacement(
        &self,
        kms: &KMS,
        candidates: &[RotationCandidate],
    ) -> impl std::future::Future<Output = KResult<Vec<ReplacementObject>>>;
    ...
    fn rewrap_new_objects(
        ...
        default_rewrap_new_objects(kms, user, replacements, wrap_specs)
    }
    ...
    fn persist_new_key(
        ...
        default_persist_new_key(kms, user, replacements)
    }
    ...
    fn finalize_dependants(
        ...
        default_finalize_dependants(kms, user, candidates, replacements)
    }

    /// Step 8: Build the KMIP response from the completed replacements.
    fn build_response(&self, replacements: &[ReplacementObject]) -> Self::Response;
}

/// Default implementation for [`RekeyOperation::persist_new_key`].
///
/// Creates all replacement objects in a single atomic database transaction.
async fn default_persist_new_key(
    kms: &KMS,
    user: &str,
    replacements: &[ReplacementObject],
) -> KResult<()> {
