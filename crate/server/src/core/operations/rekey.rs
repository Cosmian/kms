...
use crate::{
    core::{
        KMS, operations::key_ops::ObjectWithMetadataOps,
        retrieve_object_utils::user_has_permission, wrapping::unwrap_object,
    },
    error::KmsError,
    kms_bail,
...
pub(crate) async fn rekey(
        ...
        .into_values()
    {
        ...
        // Per KMIP §4.4: the replacement key inherits the Name from the existing key.
        if new_key_attrs.name.is_none() {
            new_key_attrs.name.clone_from(&owm.attributes().name);
        }
        ...
        // Per KMIP §4.4: the Name is transferred to the replacement key.
        // Remove it from the old key so Locate-by-name returns the new key.
        old_key_attrs.name = None;
