    ...
    for requested in req_attributes {
        match requested {
            ...
            AttributeReference::Standard(tag) => match tag {
                ...
                Tag::Digest => {
                    // For symmetric keys, recompute digest over raw key bytes and report
                    // KeyFormatType=Raw to match profile vectors (e.g., TL-M-3-21).
                    ...
                    } else if let Some(digest) = attributes.digest.clone() {
