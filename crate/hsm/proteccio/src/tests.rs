...
fn test_hsm_proteccio_all() -> HResult<()> {
    ...
    // cycles. The Proteccio native library (libnethsm.so) is not safely re-initializable
    // within the same process: repeated load/unload cycles corrupt internal C state and
    // cause a SIGSEGV (signal 11) on subsequent PKCS#11 calls.
    let cfg = cfg()?;
    let hsm = shared::instantiate::<ProteccioCapabilityProvider>(&cfg)?;
    drop(hsm.hsm_lib().get_info_struct()?);
    let slot = shared::get_slot::<ProteccioCapabilityProvider>(&hsm, &cfg)?;
