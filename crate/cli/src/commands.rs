...
// use cosmian_kms_client::{reexport::cosmian_config_utils::ConfigUtils, KmsClient, KmsClientConfig};
...
impl KmsOptions {
    ...
    pub fn prepare_config(&self) -> CliResult<KmsClientConfig> {
        ...
        if self.url.is_some() {
            ...
            config.http_config.server_url = self.url.clone().unwrap_or_default();
        }
        if self.accept_invalid_certs.is_some() {
            ...
            config.http_config.accept_invalid_certs = self.accept_invalid_certs.unwrap_or_default();
        }
        if self.print_json {
            info!(
