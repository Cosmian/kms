...
impl BenchAction {
    ...
    pub async fn process(&self, kms_rest_client: Arc<KmsClient>) -> CosmianResult<()> {
        ...
        let format = CustomFormat::builder()
            ...
            .separator(" ")
            .build()
            .map_err(|e| CosmianError::Default(format!("Failed to create the formatter: {e}")))?;
        ...
        for handle in handles {
            match handle.await {
                ...
                Err(e) => return Err(CosmianError::Default(format!("Tokio Error: {e}"))),
            }
        }
...
async fn encrypt(
    ...
    num_batches: usize,
) -> CosmianResult<Vec<EncryptionResult>> {
