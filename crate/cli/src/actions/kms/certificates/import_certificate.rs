...
impl ImportCertificateAction {
        ...
        let (stdout_message, returned_unique_identifier) = match self.input_format {
            ...
            CertificateInputFormat::Pem => {
                ...
                let certificate = Certificate::from_pem(&pem_value).map_err(|e| {
                    CosmianError::Conversion(format!(
            ...
            CertificateInputFormat::CCADB => {
                let ccadb_bytes = reqwest::get(MOZILLA_CCADB)
                    .await
                    .map_err(|e| {
                        CosmianError::ItemNotFound(format!(
    ...
    fn get_certificate_file(&self) -> CosmianResult<&PathBuf> {
        self.certificate_file.as_ref().ok_or_else(|| {
            CosmianError::InvalidRequest(format!(
...
fn build_chain_from_stack(pem_chain: &[u8]) -> CosmianResult<Vec<Object>> {
    let pem_s = pem::parse_many(pem_chain)
        .map_err(|e| CosmianError::Conversion(format!("Cannot parse PEM content. Error: {e:?}")))?; // check the PEM is valid (no error
    let mut objects = vec![];
