...
pub(crate) fn compare_object(expected: &Object, actual: &Object) -> KmsCliResult<()> {
    ...
    match (expected, actual) {
        (Object::Certificate(e), Object::Certificate(a)) => {
            if e.certificate_type != a.certificate_type {
                return Err(KmsCliError::Default(format!(
                    "Certificate.certificate_type expected={:?} actual={:?}",
                    e.certificate_type, a.certificate_type
                )));
            }
            if e.certificate_value != a.certificate_value {
                return Err(KmsCliError::Default(format!(
                    "Certificate.certificate_value mismatch expected_len={} actual_len={}",
                    e.certificate_value.len(),
                    a.certificate_value.len()
                )));
            }
        }
        (Object::CertificateRequest(e), Object::CertificateRequest(a)) => {
            if e.certificate_request_type != a.certificate_request_type {
                return Err(KmsCliError::Default(format!(
                    "CertificateRequest.certificate_request_type expected={:?} actual={:?}",
                    e.certificate_request_type, a.certificate_request_type
                )));
            }
            if e.certificate_request_value != a.certificate_request_value {
                return Err(KmsCliError::Default(format!(
                    "CertificateRequest.certificate_request_value mismatch expected_len={} actual_len={}",
                    e.certificate_request_value.len(),
                    a.certificate_request_value.len()
                )));
            }
        }
        (Object::OpaqueObject(e), Object::OpaqueObject(a)) => {
            if e.opaque_data_type != a.opaque_data_type {
                return Err(KmsCliError::Default(format!(
                    "OpaqueObject.opaque_data_type expected={:?} actual={:?}",
                    e.opaque_data_type, a.opaque_data_type
                )));
            }
            if e.opaque_data_value != a.opaque_data_value {
                return Err(KmsCliError::Default(format!(
                    "OpaqueObject.opaque_data_value mismatch expected_len={} actual_len={}",
                    e.opaque_data_value.len(),
                    a.opaque_data_value.len()
                )));
            }
        }
...
pub(crate) fn compare_key_block(
    ...
    if expected.key_format_type != actual.key_format_type {
        use kmip_2_1::kmip_types::KeyFormatType;
        let tolerant = expected.key_format_type == KeyFormatType::TransparentSymmetricKey
            && actual.key_format_type == KeyFormatType::Raw;
        if !tolerant {
            return Err(KmsCliError::Default(format!(
                "KeyBlock.key_format_type expected={} actual={}",
                expected.key_format_type, actual.key_format_type
            )));
        }
    }
