...
pub(crate) fn prepare_attributes(common_attributes: &CommonAttributes) -> Vec<std::string::String> {
    let mut args = Vec::<String>::new();

    if let Some(id) = &common_attributes.id {
        args.push("--id".to_owned());
        args.push(id.clone());
    }
    if let Some(date) = common_attributes.activation_date {
    ...
    if let Some(cryptographic_length) = common_attributes.cryptographic_length {
        args.push("--cryptographic-length".to_owned());
        args.push(cryptographic_length.to_string());
    }

    if let Some(key_usage_vec) = &common_attributes.key_usage {
    ...
    if let Some(public_key_id) = &common_attributes.public_key_id {
        args.push("--public-key-id".to_owned());
        args.push(public_key_id.clone());
    }

    if let Some(private_key_id) = &common_attributes.private_key_id {
        args.push("--private-key-id".to_owned());
        args.push(private_key_id.clone());
    }

    if let Some(certificate_id) = &common_attributes.certificate_id {
        args.push("--certificate-id".to_owned());
        args.push(certificate_id.clone());
    }

    if let Some(pkcs12_certificate_id) = &common_attributes.pkcs12_certificate_id {
        args.push("--pkcs12-certificate-id".to_owned());
        args.push(pkcs12_certificate_id.clone());
    }

    if let Some(pkcs12_password_certificate) = &common_attributes.pkcs12_password_certificate {
        args.push("--pkcs12-password-certificate".to_owned());
        args.push(pkcs12_password_certificate.clone());
    }

    if let Some(parent_id) = &common_attributes.parent_id {
        args.push("--parent-id".to_owned());
        args.push(parent_id.clone());
    }

    if let Some(child_id) = &common_attributes.child_id {
        args.push("--child-id".to_owned());
        args.push(child_id.clone());
    }

    if let Some(state) = common_attributes.state {
        args.push("--state".to_owned());
        args.push(state.to_string());
    }

    if let Some(tags) = &common_attributes.tags {
        for tag in tags {
    ...
    if let Some(vendor_attributes) = &common_attributes.vendor_attributes {
...
pub(crate) fn set_attributes(
    ...
    let args = prepare_attributes(common_attributes);

    let mut cmd = Command::cargo_bin(PROG_NAME)?;
    cmd.env(KMS_CLI_CONF_ENV, cli_conf_path);

    cmd.arg("set-attributes").args(args);
