...
pub async fn test_import_export_wrap_rfc_5649() -> Result<(), CliError> {
    ...
    let wrap_key_uid = import("sym", wrap_key_path.to_str().unwrap(), None, false, false).await?;
    ...
    .await?;
    test_import_export_wrap_private_key("cc", &private_key_id, &wrap_key_uid, &wrap_key).await?;
    // test ec
    let (private_key_id, _public_key_id) =
        elliptic_curve::create_key_pair::create_ec_key_pair().await?;
    test_import_export_wrap_private_key("ec", &private_key_id, &wrap_key_uid, &wrap_key).await?;
    // test sym
    let key_id = symmetric::create_key::create_symmetric_key(None, None, None).await?;
    test_import_export_wrap_private_key("sym", &key_id, &wrap_key_uid, &wrap_key).await?;
