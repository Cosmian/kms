...
use crate::{
    ...
    error::CliError,
    tests::{
        rsa::create_key_pair::create_rsa_key_pair,
...
async fn test_rsa_encrypt_decrypt_using_ids() -> Result<(), CliError> {
    ...
    let (private_key_id, public_key_id) = create_rsa_key_pair(&ctx.owner_cli_conf_path, &[])?;
...
async fn test_rsa_encrypt_decrypt_using_tags() -> Result<(), CliError> {
    ...
    let (_private_key_id, _public_key_id) =
        create_rsa_key_pair(&ctx.owner_cli_conf_path, &["tag_rsa"])?;
