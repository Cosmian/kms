...
pub(crate) fn create_mac(cli_conf_path: &str, action: MacAction) -> CliResult<String> {
    ...
    let mut args = vec!["--mac-key-id".to_owned(), action.mac_key_id.clone()];
    if let Some(algorithm) = action.hashing_algorithm {
        args.extend(vec!["--algorithm".to_owned(), algorithm.to_string()]);
    }
    if let Some(data) = action.data.clone() {
        args.extend(vec!["--data".to_owned(), data]);
    }
    if let Some(correlation_value) = action.correlation_value.clone() {
...
pub(crate) async fn test_mac() -> CliResult<()> {
    ...
    create_mac(
        ...
        MacAction {
            mac_key_id,
            hashing_algorithm: Some(CHashingAlgorithm::SHA3_256),
