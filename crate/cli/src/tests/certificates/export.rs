...
use crate::{
    ...
    tests::{
        PROG_NAME,
        certificates::{
            certify::{CertifyOp, certify, create_self_signed_cert, import_root_and_intermediate},
...
async fn test_import_export_p12_25519() {
    ...
    let cert_attributes = Attributes::try_from_ttlv(&cert_attributes_ttlv).unwrap();
    ...
    let issuer_cert_attributes = Attributes::try_from_ttlv(&issuer_cert_attributes_ttlv).unwrap();
    assert!(
