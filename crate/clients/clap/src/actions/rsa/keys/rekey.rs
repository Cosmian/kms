...
///
/// Creates a new RSA key pair with a fresh UUID, preserving the rotation
/// policy and linking the old key via `ReplacedObjectLink`.
#[derive(Parser)]
#[clap(verbatim_doc_comment)]
pub struct ReKeyAction {
    /// The unique identifier of the RSA private key to rotate.
...
impl ReKeyAction {
    pub(crate) async fn run(&self, kms_rest_client: KmsClient) -> KmsCliResult<UniqueIdentifier> {
        ...
        let unique_identifier = kms_rest_client
            ...
            .await
            .with_context(|| "failed rekeying the RSA key pair")?
            .unique_identifier;

        let mut stdout = console::Stdout::new("The RSA key pair was successfully refreshed.");
