...
async fn ecdsa_digested_sign_verify_cli_end_to_end() -> crate::error::result::KmsCliResult<()> {
    ...
    let (private_key_id, public_key_id) =
        crate::actions::elliptic_curves::keys::create_key_pair::CreateKeyPairAction::default()
            .run(ctx.get_owner_client())
            .await?;

    // Use CLI SignAction with --digested path and explicit output
    crate::actions::elliptic_curves::sign::SignAction {
        curve:
    ...
    let validity = crate::actions::elliptic_curves::signature_verify::SignatureVerifyAction {
