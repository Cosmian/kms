...
impl Object {
    ...
    pub fn attribute(&self, type_: AttributeType) -> ModuleResult<Option<Attribute>> {
        let attribute = match self {
            ...
            Self::PrivateKey(private_key) => private_key_attribute(private_key.as_ref(), type_)?,
            Self::Profile(id) => profile_attribute(*id, type_),
            Self::PublicKey(pk) => public_key_attribute(pk.as_ref(), type_)?,
            Self::DataObject(data) => data_object_attribute(data.as_ref(), type_),
        };
        debug!(
            "Object: {}, attribute: {:?} => {:?}",
            self.name(),
...
/// Centralises the three-step conversion (DER → `PKey` → `Rsa`) that is needed
/// for `Modulus`, `PublicExponent`, and `Value` attribute lookups.
fn rsa_from_private_key(key: &dyn PrivateKey) -> ModuleResult<Rsa<Private>> {
    let der_bytes = key.pkcs8_der_bytes()?;
    let pkey = PKey::private_key_from_der(der_bytes.as_ref()).map_err(|e| {
        error!("Failed to parse RSA private key from PKCS#8 DER: {e:?}");
        ModuleError::Cryptography("Failed to parse RSA private key from PKCS#8 DER".to_owned())
    })?;
    pkey.rsa().map_err(|e| {
...
fn certificate_attribute(
    cert: &dyn Certificate,
    type_: AttributeType,
) -> ModuleResult<Option<Attribute>> {
    Ok(match type_ {
        ...
        AttributeType::SerialNumber => cert.serial_number().map(Attribute::SerialNumber).ok(),
        ...
        AttributeType::Modulus => Some(Attribute::Modulus(cert.public_key()?.rsa_modulus()?)),
...
fn sym_key_attribute(
    sym_key: &dyn SymmetricKey,
    type_: AttributeType,
) -> ModuleResult<Option<Attribute>> {
    Ok(match type_ {
        ...
        AttributeType::KeyType => Some(Attribute::KeyType(sym_key.algorithm().to_ck_key_type())),
...
fn private_key_attribute(
    ...
    Ok(match type_ {
        ...
        AttributeType::Decrypt => Some(Attribute::Decrypt(true)),
        AttributeType::EcParams => match private_key.algorithm() {
        ...
        AttributeType::Extractable => Some(Attribute::Extractable(false)),
        AttributeType::Id => Some(Attribute::Id(private_key.remote_id().as_bytes().to_vec())),
        ...
        AttributeType::PublicExponent => Some(Attribute::PublicExponent(
            rsa_from_private_key(private_key)?.e().to_vec(),
        )),
        AttributeType::Sensitive => Some(Attribute::Sensitive(true)),
        ...
        AttributeType::Value => match private_key.algorithm() {
            KeyAlgorithm::Rsa => rsa_from_private_key(private_key)?
                .private_key_to_der()
                .map(Attribute::Value)
                .ok(),
            KeyAlgorithm::EccP256
...
fn profile_attribute(id: CK_PROFILE_ID, type_: AttributeType) -> Option<Attribute> {
    match type_ {
...
fn data_object_attribute(data: &dyn DataObject, type_: AttributeType) -> Option<Attribute> {
    match type_ {
