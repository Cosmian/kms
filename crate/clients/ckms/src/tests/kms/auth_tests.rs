...
/// Test that the KMS server correctly negotiates a PQC-compatible TLS 1.3 cipher suite.
///
/// `TLS_AES_256_GCM_SHA384` is the symmetric cipher mandated for TLS 1.3 hybrid
/// post-quantum key exchange (e.g., X25519MLKEM768 per NIST SP 800-227).  This
/// test restricts the server to that cipher only and verifies that the CLI can
/// still authenticate and list owned keys successfully.
#[allow(clippy::large_stack_frames)]
#[tokio::test]
pub(crate) async fn test_kms_pqc_tls_cipher_suite() -> CosmianResult<()> {
    log_init(None);
    ...
    let db_config = MainDBConfig {
        database_type: Some("sqlite".to_owned()),
        sqlite_path: PathBuf::from("./sqlite-data-pqc-tls-tests"),
        clear_database: true,
        ..MainDBConfig::default()
    };

    // Restrict the server to TLS_AES_256_GCM_SHA384: the TLS 1.3 cipher used
    // with PQC hybrid key-exchange algorithms.  The client uses standard TLS
    // negotiation so OpenSSL selects a mutually supported version/cipher.
    info!(
        "==> Testing server restricted to PQC-compatible TLS 1.3 cipher \
    ...
    let ctx = start_test_server_with_options(
        ...
        AuthenticationOptions {
            ...
            server_params: Some(build_server_params(
                db_config.clone(),
                port,
                // HTTPS without requiring a client certificate so we test
                // cipher-suite negotiation in isolation.
                TlsMode::HttpsNoClientCa,
                JwtAuth::Disabled,
                // Restrict server to the PQC-grade TLS 1.3 cipher only.
                Some("TLS_AES_256_GCM_SHA384".to_owned()),
                None,
            )?),
        },
        None,
