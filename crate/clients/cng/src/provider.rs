...
/// Enforces a maximum length of 32KB to prevent unbounded reads of malformed pointers.
unsafe fn wide_ptr_to_string(ptr: *const u16) -> KspResult<String> {
    const MAX_STR_LEN: usize = 32 * 1024; // 32KB max
    if ptr.is_null() {
        return Err(KspError::InvalidParameter("null wide string".to_owned()));
    }
    let mut len = 0_usize;
    unsafe {
        while len < MAX_STR_LEN && *ptr.add(len) != 0 {
            len += 1;
        }
    }
    if len >= MAX_STR_LEN {
        return Err(KspError::InvalidParameter(
            "wide string exceeds maximum length".to_owned(),
        ));
    }
    #[allow(clippy::as_conversions)]
...
unsafe extern "system" fn free_provider(h_provider: usize) -> SecurityStatus {
    ...
    // Validate magic before freeing to catch stale/invalid handles
    if ptr.is_null() || unsafe { (*ptr).magic } != PROVIDER_CTX_MAGIC {
        return NTE_INVALID_HANDLE;
    }
...
/// For NCryptKeyName structures, frees both the struct and its name buffer.
unsafe extern "system" fn free_buffer(pv_input: *mut core::ffi::c_void) -> SecurityStatus {
    if pv_input.is_null() {
        return ERROR_SUCCESS;
    }
    // Free as NCryptKeyName struct: internally frees pszName buffer and struct
    let key_name_ptr = pv_input.cast::<NCryptKeyName>();
    unsafe {
        if let Some(key_name) = key_name_ptr.as_ref() {
            // Free the embedded name buffer if present
            if !key_name.pszName.is_null() {
                let _name_box = Box::from_raw(key_name.pszName.cast::<[u16; 0]>());
            }
        }
        // Free the struct itself
        let _struct_box = Box::from_raw(key_name_ptr);
    }
    ERROR_SUCCESS
}
...
unsafe extern "system" fn enum_keys(
    ...
    // Allocate a proper NCRYPT_KEY_NAME struct with the key name.
    // The struct contains: pszName (*const u16), pszAlgid (*const u16), dwLegacyKeySpec (u32), dwFlags (u32)
    let wide_name: Vec<u16> = name.encode_utf16().chain(std::iter::once(0)).collect();
    let leaked_name = Box::into_raw(wide_name.into_boxed_slice());

    let key_name = NCryptKeyName {
        pszName: leaked_name.cast::<u16>(),
        pszAlgid: std::ptr::null_mut(),
        dwLegacyKeySpec: 0,
        dwFlags: 0,
    };

    let key_name_box = Box::new(key_name);
    let key_name_ptr = Box::into_raw(key_name_box);
    unsafe {
        *pp_key_name = key_name_ptr;
    }
