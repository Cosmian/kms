...
use crate::{
    crypto::{
        ...
        wrap::{rfc_5649_unwrap, tfc_5649_wrap},
    },
...
pub fn wrap_key_bytes(key: &[u8], wrapping_password: &str) -> Result<Vec<u8>, KmipUtilsError> {
    ...
    rfc_5649_wrap(key, wrapping_secret.as_ref()).map_err(|e| KmipUtilsError::Default(e.to_string()))
}
...
pub fn unwrap_key_bytes(
    ...
    let wrapping_secret =
        derive_key_from_password::<WRAPPING_SECRET_LENGTH>(wrapping_password.as_bytes())?;
    rfc_5649_unwrap(key, wrapping_secret.as_ref()).map_err(|e| KmipUtilsError::Default(e.to_string()))
}
