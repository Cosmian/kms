...
use pkcs11_module::traits::{Certificate, PublicKey};
use x509_cert::{
    der::{Decode, Encode},
...
impl Certificate for Pkcs11Certificate {
    ...
    fn to_der(&self) -> pkcs11_module::Result<Vec<u8>> {
        self.certificate
            .to_der()
    ...
    fn issuer(&self) -> pkcs11_module::Result<Vec<u8>> {
        Encode::to_der(&self.certificate.tbs_certificate.issuer)
            .map_err(|e| Pkcs11Error::from(e).into())
    }

    fn serial_number(&self) -> pkcs11_module::Result<Vec<u8>> {
        Encode::to_der(&self.certificate.tbs_certificate.serial_number)
            .map_err(|e| Pkcs11Error::from(e).into())
    }

    fn subject(&self) -> pkcs11_module::Result<Vec<u8>> {
