...
use crate::{
    // error::{result::CryptoResultHelper, KmipError},
    crypto::{
...
pub fn encrypt_bytes(wrapping_key: &Object, plaintext: &[u8]) -> Result<Vec<u8>, KmipError> {
    debug!(
    ...
    match wrapping_key {
        Object::Certificate {
            ...
            let ciphertext = encrypt_response.data.ok_or(KmipError::Default(
...
fn encrypt_with_public_key(pubkey: PKey<Public>, plaintext: &[u8]) -> Result<Vec<u8>, KmipError> {
    let request = Encrypt {
        data: Some(plaintext.to_vec()),
        ..Encrypt::default()
    };
...
fn decrypt_with_private_key(
    ...
    let plaintext = Zeroizing::from(decrypt_response.data.ok_or(KmipError::Default(
