...
fn ecies_get_iv(Q: &EcPointRef, R: &EcPointRef, curve: &EcGroupRef) -> Result<Vec<u8>, KmipError> {
...
#[allow(non_snake_case)]
fn ecies_get_key(S: &EcPointRef, curve: &EcGroupRef) -> Result<Zeroizing<Vec<u8>>, KmipError> {
    let mut ctx = BigNumContext::new_secure()?;
...
pub fn ecies_encrypt(pubkey: &PKey<Public>, plaintext: &[u8]) -> Result<Vec<u8>, KmipError> {
    ...
    if curve.curve_name() == Some(Nid::X9_62_PRIME192V1) {
        kmip_bail!("ECIES: Curve P-192 not allowed in FIPS mode.")
    }
