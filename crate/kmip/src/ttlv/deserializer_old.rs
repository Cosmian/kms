//! TTLV Format Deserialization implementation.
//!
...
//!
//! - `TtlvDeserializerOld`: Core deserializer that implements the Serde `Deserializer` trait
...
///
/// #see `Object::post_fix()`
pub fn from_ttlv_old<'a, T>(s: &'a TTLV) -> Result<T>
where
    T: Deserialize<'a>,
{
    ...
    let mut deserializer = TtlvDeserializerOld::from_ttlv(s);
...
#[derive(Debug)]
pub struct TtlvDeserializerOld<'de> {
...
impl<'de> TtlvDeserializerOld<'de> {
...
impl<'de> de::Deserializer<'de> for &mut TtlvDeserializerOld<'de> {
        ...
        V: Visitor<'de>,
    {
        ...
        match child_value {
            TTLValue::Structure(elements) => {
                ...
                let ttlv_de = TtlvDeserializerOld {
                    deserializing: Deserializing::StructureValue,
            ...
            TTLValue::BigInteger(e) => visitor.visit_seq(TtlvDeserializerOld {
        ...
        match self.deserializing {
            ...
            Deserializing::StructureValue => {
                ...
                match value {
                    ...
                    {
                        visitor.visit_seq(TtlvDeserializerOld {
                            deserializing: Deserializing::ByteString,
        ...
        match &self.deserializing {
            Deserializing::StructureValue => {
                ...
                match value {
                    TTLValue::Structure(array) => {
                        // go down one level by deserializing the inner structure
                        visitor.visit_map(TtlvDeserializerOld {
                            deserializing: Deserializing::StructureValue,
                            input: Input::Structure(array.iter().collect::<Vec<&TTLV>>()), // can probably do better
                            // start at 0 because the Visit Map is going to increment first
                            index: 0,
...
impl<'de> MapAccess<'de> for TtlvDeserializerOld<'de> {
    type Error = TtlvError;
...
impl<'de> SeqAccess<'de> for TtlvDeserializerOld<'de> {
    type Error = TtlvError;
...
struct EnumWalker<'a, 'de: 'a> {
    de: &'a mut TtlvDeserializerOld<'de>,
}

impl<'a, 'de> EnumWalker<'a, 'de> {
    // #[instrument(skip(de))]
    fn new(de: &'a mut TtlvDeserializerOld<'de>) -> Self {
        EnumWalker { de }
    }
}
