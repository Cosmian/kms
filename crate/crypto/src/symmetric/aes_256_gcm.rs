...
use crate::{error::KmsCryptoError, kms_crypto_bail, DecryptionSystem, EncryptionSystem};
...
impl AesGcmSystem {
    pub fn instantiate(uid: &str, symmetric_key: &Object) -> Result<Self, KmsCryptoError> {
        let key_block = match symmetric_key {
            Object::SymmetricKey { key_block } => key_block,
            _ => {
                return Err(KmsCryptoError::NotSupported(
        ...
        if symmetric_key.len() != AES_256_GCM_KEY_LENGTH {
            kms_crypto_bail!("Expected a KMIP Symmetric Key of length {AES_256_GCM_KEY_LENGTH}")
        }
...
impl DecryptionSystem for AesGcmSystem {
    fn decrypt(&self, request: &Decrypt) -> Result<DecryptResponse, KmsCryptoError> {
        ...
        let request_nonce_bytes = request.iv_counter_nonce.as_ref().ok_or_else(|| {
            KmsCryptoError::NotSupported("The nonce is mandatory for AES GCM.".to_string())
        })?;
