...
pub fn key_wrap(plain: &[u8], kek: &[u8]) -> Result<Vec<u8>, KmsCryptoError> {
    let n = plain.len();
    let n_bytes_key = n % 8;
    ...
    if n <= 8 {
        ...
        let ciphertext = match kek.len() {
            ...
            _ => {
                return Err(KmsCryptoError::InvalidSize(
...
/// The function name matches the one used in the RFC and has no link to the
/// unwrap function in Rust.
pub fn key_unwrap(ciphertext: &[u8], kek: &[u8]) -> Result<Zeroizing<Vec<u8>>, KmsCryptoError> {
...
/// The function name matches the one used in the RFC and has no link to the
/// unwrap function in Rust.
fn _wrap_64(plain: &[u8], kek: &[u8], iv: Option<u64>) -> Result<Vec<u8>, KmsCryptoError> {
    let n = plain.len();
...
fn _unwrap_64(ciphertext: &[u8], kek: &[u8]) -> Result<(u64, Zeroizing<Vec<u8>>), KmsCryptoError> {
    let n = ciphertext.len();
...
mod tests {
    ...
    use crate::wrap::rfc5649::{key_unwrap, key_wrap};
