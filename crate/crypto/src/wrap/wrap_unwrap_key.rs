...
use crate::{
    error::{result::CryptoResultHelper, KmsCryptoError},
    kms_crypto_bail,
...
pub fn wrap_key_bytes(key: &[u8], wrapping_password: &str) -> Result<Vec<u8>, KmsCryptoError> {
    let wrapping_secret =
        derive_key_from_password::<WRAPPING_SECRET_LENGTH>(wrapping_password.as_bytes())?;
    key_wrap(key, wrapping_secret.as_ref()).map_err(|e| KmsCryptoError::Default(e.to_string()))
}
...
pub fn unwrap_key_bytes(
    ...
    let wrapping_secret =
        derive_key_from_password::<WRAPPING_SECRET_LENGTH>(wrapping_password.as_bytes())?;
    key_unwrap(key, wrapping_secret.as_ref()).map_err(|e| KmsCryptoError::Default(e.to_string()))
}
...
pub fn unwrap_key_block(
    ...
    if WrappingMethod::Encrypt != key_wrapping_data.wrapping_method {
        kms_crypto_bail!("unable to unwrap key: only the Encrypt unwrapping method is supported")
    }
...
mod tests {
    ...
    #[test]
    fn test_wrap_unwrap() -> Result<(), KmsCryptoError> {
