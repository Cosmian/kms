...
use crate::{
    cover_crypt::attributes::policy_from_attributes, error::KmsCryptoError,
    generic::data_to_encrypt::DataToEncrypt, EncryptionSystem,
};
...
impl CoverCryptEncryption {
    ...
    fn encrypt(
        ...
        let encrypted_block = self
            ...
            .map_err(|e| {
                KmsCryptoError::Kmip(ErrorReason::Invalid_Attribute_Value, e.to_string())
            })?;
...
impl EncryptionSystem for CoverCryptEncryption {
    fn encrypt(&self, request: &Encrypt) -> Result<EncryptResponse, KmsCryptoError> {
        ...
        .map_err(|e| KmsCryptoError::Kmip(ErrorReason::Invalid_Attribute_Value, e.to_string()))?;
