...
#[cfg(not(feature = "fips"))]
use crate::elliptic_curves::operation::{ED25519_PRIVATE_KEY_LENGTH, X25519_PRIVATE_KEY_LENGTH};
use crate::{
...
impl DecryptionSystem for HybridDecryptionSystem {
    fn decrypt(&self, request: &Decrypt) -> Result<DecryptResponse, KmsCryptoError> {
        ...
        let plaintext = match self.private_key.id() {
            ...
            x => {
                kms_crypto_bail!("private key id not supported yet: {:?}", x);
            }
        };
