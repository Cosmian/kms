...
#[cfg(not(feature = "fips"))]
use crate::elliptic_curves::operation::{ED25519_PUBLIC_KEY_LENGTH, X25519_PUBLIC_KEY_LENGTH};
use crate::{
...
impl HybridEncryptionSystem {
    ...
    pub fn instantiate_with_certificate(
        ...
        let cert = X509::from_der(certificate_value)
            .map_err(|e| KmsCryptoError::ConversionError(format!("invalid X509 DER: {e:?}")))?;

        debug!("instantiate_with_certificate: get the public key of the certificate");
        let public_key = cert.public_key().map_err(|e| {
            KmsCryptoError::ConversionError(format!("invalid public key: error: {e:?}"))
        })?;
