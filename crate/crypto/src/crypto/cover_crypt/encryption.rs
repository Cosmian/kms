...
impl EncryptionSystem for CoverCryptEncryption {
    fn encrypt(&self, request: &Encrypt) -> Result<EncryptResponse, CryptoError> {
        let ad = request.ad.as_deref().ok_or_else(|| {
            CryptoError::Kmip(
                "The decryption request should contain additional encrypted data".to_owned(),
            )
        })?;
        ...
        let symmetric_key = SymmetricKey::derive(&secret, ad)?;
        ...
        }) = request.cryptographic_parameters
        {
            ...
            let mut encrypted_data =
                self.encrypt(&data_to_encrypt.plaintext, Some(ad), &symmetric_key)?;
        ...
        Ok(EncryptResponse {
            ...
            authenticated_encryption_tag: Some(ad.to_vec()),
        })
    }
}
