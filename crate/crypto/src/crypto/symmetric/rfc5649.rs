...
const AES_BLOCK_SIZE: usize = 128; // 128-bit
...
pub fn rfc5649_wrap(plaintext: &[u8], kek: &[u8]) -> CryptoResult<Vec<u8>> {
    ...
    // Minimum output is 16 bytes (2 semiblocks)
    // The wrapped size includes the AIV (8 bytes) plus padded plaintext
    let padded_len = if n_bytes <= AES_WRAP_BLOCK_SIZE {
        AES_BLOCK_SIZE // Special case: single block encryption
    } else {
        // Calculate padding: round up to next multiple of 8, then add 8 for AIV
        let padded_plaintext_len =
            ((n_bytes + AES_WRAP_BLOCK_SIZE - 1) / AES_WRAP_BLOCK_SIZE) * AES_WRAP_BLOCK_SIZE;
        padded_plaintext_len + AES_WRAP_BLOCK_SIZE
    };
...
pub fn rfc5649_unwrap(ciphertext: &[u8], kek: &[u8]) -> CryptoResult<Zeroizing<Vec<u8>>> {
    ...
    // RFC 5649 requires ciphertext to be at least 16 bytes (minimum 2 semiblocks) and a multiple of 8
...
// pub fn rfc5649_wrap(plain: &[u8], kek: &[u8]) -> Result<Vec<u8>, CryptoError> {
//     let n = plain.len();
//     let n_bytes_key = n % 8;
