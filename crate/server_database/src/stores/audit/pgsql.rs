...
/// The audit writer is a single task awaiting one `write_event` at a time, so it can
/// never use a second connection. The pool exists so `write_event`'s retry loop can ask
...
mod live_tests {
    ...
    async fn pg_resume_rejects_interior_tamper() {
        ...
        sink.write_event(&ev2).await.unwrap();
        drop(sink);
