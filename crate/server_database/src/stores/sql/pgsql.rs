...
impl PgPool {
    pub(crate) async fn instantiate(
        ...
        // Phase 1 — Schema: real PRIMARY KEYs for logical replication (PostgreSQL only)
        // `tags` and `read_access` have UNIQUE constraints on nullable columns by default.
        // Logical replication requires a non-nullable PRIMARY KEY as REPLICA IDENTITY.
        let tags_has_pk = client
            .query_one(
                "SELECT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tags_pkey');",
                &[],
            )
            .await
            .map_err(DbError::from)?
        ...
        if !tags_has_pk {
            client
                .batch_execute("ALTER TABLE tags ALTER COLUMN id SET NOT NULL;")
                .await
                .map_err(DbError::from)?;
            client
                .batch_execute("ALTER TABLE tags ALTER COLUMN tag SET NOT NULL;")
                .await
                .map_err(DbError::from)?;
            client
                .batch_execute(
                    "CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_tags_pkey ON tags (id, tag);",
                )
                .await
                .map_err(DbError::from)?;
            client
                .batch_execute("ALTER TABLE tags ADD CONSTRAINT tags_pkey PRIMARY KEY USING INDEX idx_tags_pkey;")
                .await
                .map_err(DbError::from)?;
            client
                .batch_execute("ALTER TABLE tags DROP CONSTRAINT IF EXISTS tags_id_tag_key;")
                .await
                .map_err(DbError::from)?;
        }

        let read_access_has_pk = client
            .query_one(
                "SELECT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'read_access_pkey');",
                &[],
            )
            .await
            .map_err(DbError::from)?
            .get::<usize, bool>(0);
        if !read_access_has_pk {
            client
                .batch_execute("ALTER TABLE read_access ALTER COLUMN id SET NOT NULL;")
                .await
                .map_err(DbError::from)?;
            client
                .batch_execute("ALTER TABLE read_access ALTER COLUMN userid SET NOT NULL;")
                .await
                .map_err(DbError::from)?;
            client
                .batch_execute(
                    "CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_read_access_pkey ON read_access (id, userid);",
                )
                .await
                .map_err(DbError::from)?;
            client
                .batch_execute(
                    "ALTER TABLE read_access ADD CONSTRAINT read_access_pkey PRIMARY KEY USING INDEX idx_read_access_pkey;",
                )
                .await
                .map_err(DbError::from)?;
            client
                .batch_execute(
                    "ALTER TABLE read_access DROP CONSTRAINT IF EXISTS read_access_id_userid_key;",
                )
                .await
                .map_err(DbError::from)?;
        }

        let co_has_pk = client
            .query_one(
                "SELECT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'crypto_officer_activations_pkey');",
                &[],
            )
            .await
            .map_err(DbError::from)?
        ...
        if !co_has_pk {
            client
                .batch_execute(
                    "ALTER TABLE crypto_officer_activations ADD COLUMN IF NOT EXISTS id BIGINT GENERATED BY DEFAULT AS IDENTITY;",
                )
                .await
                .map_err(DbError::from)?;
            client
                .batch_execute(
                    "CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_co_activations_pkey ON crypto_officer_activations (id);",
                )
                .await
                .map_err(DbError::from)?;
            client
                .batch_execute(
                    "ALTER TABLE crypto_officer_activations ADD CONSTRAINT crypto_officer_activations_pkey PRIMARY KEY USING INDEX idx_co_activations_pkey;",
                )
                .await
                .map_err(DbError::from)?;
        }

        // Phase 2 — Object `State` conflict merge (PostgreSQL only, generic trigger)
        // Monotonic state guard: coercive merge to GREATEST(state) on all updates (both local
        // and replicated), protecting against cross-region write races where a local downgrade
        // write races against an already-replicated higher-rank state.
        // Marked ENABLE ALWAYS so it fires for both local and replicated sessions.
        // A narrow bypass is supported via `SET LOCAL kms.allow_backward_state_transition = 'on'`
        // for explicit KMIP batch UNDO operations.
        client
            .batch_execute(
                r"
CREATE OR REPLACE FUNCTION objects_state_monotonic_guard() RETURNS trigger AS $f$
DECLARE
  old_rank int;
  new_rank int;
  allow_undo text;
BEGIN
  allow_undo := current_setting('kms.allow_backward_state_transition', true);
  IF allow_undo = 'on' THEN
    RETURN NEW;
  END IF;

  old_rank := CASE OLD.state
    WHEN 'PreActive' THEN 1 WHEN 'Active' THEN 2 WHEN 'Deactivated' THEN 3
    WHEN 'Compromised' THEN 4 WHEN 'Destroyed' THEN 5
    WHEN 'Destroyed_Compromised' THEN 6 ELSE 0 END;
...
DROP TRIGGER IF EXISTS trg_objects_state_monotonic ON objects;
CREATE TRIGGER trg_objects_state_monotonic
  BEFORE UPDATE ON objects
  FOR EACH ROW
  WHEN (NEW.state IS DISTINCT FROM OLD.state)
  EXECUTE FUNCTION objects_state_monotonic_guard();

ALTER TABLE objects ENABLE ALWAYS TRIGGER trg_objects_state_monotonic;
",
            )
            .await
            .map_err(DbError::from)?;
    ...
    #[cfg(test)]
    pub(crate) const fn raw_pool(&self) -> &Pool {
        &self.pool
    }
}
