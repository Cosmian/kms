...
// removed unused Arc import after dropping SessionParams
...
use rawsql::Loader;
// Use collect instead of try_collect to avoid extra dependency
...
macro_rules! get_pgsql_query {
    ($name:literal) => {
        PGSQL_QUERIES.get($name).expect("Missing SQL query")
    };
}
