...
pkgs228.mkShell {
  ...
  ++ (
    if withPython then
      ...
      let
        pyBase = if pkgs228 ? python311 then pkgs228.python311 else pkgs.python311;
        pyVenv = if (pkgs228 ? python311Packages) && (pkgs228.python311Packages ? virtualenv) then pkgs228.python311Packages.virtualenv else pkgs.python311Packages.virtualenv;
      in
      [
        pyBase
        pyVenv
