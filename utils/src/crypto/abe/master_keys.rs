...
pub fn create_master_keypair(request: &CreateKeyPair) -> LibResult<KeyPair> {
    let attributes = request
        ...
        .ok_or_else(|| {
            LibError::Error("Attributes must be provided in a CreateKeyPair request".to_owned())
        })
    ...
    let mk = engine.generate_master_key(&policy)?;
    ...
    let private_key =
        create_master_private_key_object(&mk.0.as_bytes()?, &policy, private_key_attributes)?;
    ...
    let public_key_attributes = request
        ...
        .or(request.common_attributes.as_ref());
    let public_key =
        create_master_public_key_object(&mk.1.as_bytes()?, &policy, public_key_attributes)?;
