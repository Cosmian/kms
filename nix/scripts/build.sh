...
if [ "$(uname)" = "Linux" ]; then
  ...
  # 3) Use the Nix cc wrapper to link against the pinned glibc (ensures GLIBC<=2.40),
...
if [ "$UNAME" = "Linux" ]; then
  ...
  # Verify GLIBC symbol versions are <= 2.40 (Debian 13 compatibility)
  ...
  [ -n "$MAX_GLIBC_VER" ] && [ "$(printf '%s\n' "$MAX_GLIBC_VER" "2.40" | sort -V | tail -n1)" != "2.40" ] && {
    echo "Error: GLIBC symbols exceed 2.40 (max found: $MAX_GLIBC_VER)." >&2
    exit 1
  }
else
