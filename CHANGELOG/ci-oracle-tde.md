...
- Fix `smoke-test-tde.sh` Phase 1 CLOSED recovery: import placeholder AES-256 keys into KMS using `ckms sym keys import --key-format aes` with KMIP ID `ORACLE.TDE.HSM.MK.<ME_ID>` read from `PROPS$.VALUE$`; avoids unsafe `SYS.ENC$` manipulation
- Replace Proof 6 (REVERSE MIGRATE HSM→FILE): Oracle 23ai Free does not support `BACKUP KEYSTORE` for PKCS#11 keystores (ORA-00600: kzckmbkup: invalid keystore location [4]); replaced with **TDE REKEY** proof — rotates ME within HSM (`SET KEY`), verifies `MASTERKEYID` changes in `V$ENCRYPTED_TABLESPACES`, data readable, DBF encrypted
