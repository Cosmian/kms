...
- Change Windows installer default `$INSTDIR` from `$PROGRAMFILES64` to `$LOCALAPPDATA\Cosmian KMS Server` so that installer, wizard, and server all use the same path
