...
- **`HsmStore` prefix-aware**: `HsmStore::new()` now accepts an explicit `prefix: &str` argument. `parse_uid` uses this prefix to strip the correct routing prefix from incoming UIDs.
...
- **`GET /hsm/status` endpoint**: new HTTP endpoint (authentication required) returning a JSON array of all connected HSM instances with their prefix, model, and per-slot accessibility info.
...
- **`HsmStore::retrieve()` — sensitive-key fallback restored**: the migration from `HsmStore` to `HsmStore` accidentally dropped the issue #933 fix that falls back to `get_key_metadata()` when `export()` fails because the key is non-extractable (sensitive). The fallback now builds a metadata-only stub object so `ModifyAttribute`/`GetAttributes` succeed without touching key material. ([#942](https://github.com/Cosmian/kms/pull/942))
- **`HsmStore::update_object()` — return `Ok(())` for attribute updates**: `update_object` was returning `Err(...)` causing `ModifyAttribute`/`SetAttribute` to fail on every HSM key. HSM PKCS#11 slots do not support KMIP attribute storage; the correct behaviour is to accept the call silently (with a warning log) and return `Ok(())`. ([#942](https://github.com/Cosmian/kms/pull/942))
- **`HsmStore::find()` and `atomic()` — use `self.prefix` for UID generation**: the generated UIDs in `find()` and `atomic()` hardcoded `"hsm::"` instead of using `self.prefix`. This broke multi-HSM setups where the prefix can be `"hsm::softhsm2"`, `"hsm::utimaco"`, etc. ([#942](https://github.com/Cosmian/kms/pull/942))
...
- **`HsmStore::atomic()` — handle `UpdateObject`-only batches**: after exporting or getting an HSM key, `export_get.rs` calls `database.atomic()` with a single `UpdateObject` to persist the `fresh=false` flag. `HsmStore::atomic()` previously only handled RSA keypair creation and returned an error for any other batch, causing export/get to fail with `InvalidRequest`. It now detects all-`UpdateObject` batches and delegates each to `update_object()` (which already accepts such calls gracefully). ([#942](https://github.com/Cosmian/kms/pull/942))
...
- **SQL "obtained" access rights — show HSM keys**: the `select-objects-access-obtained` query used `INNER JOIN objects` which excluded HSM keys (they exist only in the HSM, not in the `objects` table). Changed to `LEFT JOIN` with `COALESCE` defaults so granted HSM keys appear in the "Obtained" list. ([#942](https://github.com/Cosmian/kms/pull/942))
- **`GetAttributes` permission — allow for any granted user**: `retrieve_object_utils.rs` now returns early with `Ok(true)` for `GetAttributes` when the user holds any permission on the object, matching the expected behavior where granting `Get` or `Encrypt` also implies read access to attributes. ([#942](https://github.com/Cosmian/kms/pull/942))
- **WASM `parse_selected_attributes_flatten` — handle "state" attribute**: the WASM/UI attribute parser had no handler for the `"state"` attribute, causing object state to always display as "Unknown" in the web UI Search Objects results. Added serialization of `attributes.state` so the UI receives and displays the correct lifecycle state (e.g. "Active", "PreActive"). ([#942](https://github.com/Cosmian/kms/pull/942))
- **HSM owner in "obtained" access list**: the `/access/obtained` endpoint returned an empty `owner_id` for HSM keys (not stored in the `objects` table). The server now fills in the HSM instance admin(s) from the config as owner. ([#942](https://github.com/Cosmian/kms/pull/942))

### ♻️ Refactor

- **`parse_selected_attributes_flatten` deduplication**: replaced the repetitive 5-line match arms with a local `insert_if_some!` macro, reducing the function from ~140 lines to ~50 while preserving identical behavior. ([#942](https://github.com/Cosmian/kms/pull/942))

### 🧪 Testing

- **`crate/server/src/tests/hsm/multi_hsm.rs`**: updated `test_multi_hsm_routing` — slot IDs now read from `HSM_SLOT_ID_1`/`HSM_SLOT_ID_2` env vars (were hardcoded 0/1), UID format updated to model-based scheme (`hsm::softhsm2::…`, `hsm::softhsm2_1::…`); test now pinned by `test_hsm_softhsm2.sh`; added cleanup (Destroy) of both created keys so sensitive HSM objects do not linger and cause the subsequent `test_pkcs11tool_no_warnings` check to fail. ([#942](https://github.com/Cosmian/kms/pull/942))
- **`.github/scripts/test/test_hsm_softhsm2.sh`**: added invocation of `test_multi_hsm_routing` with `HSM_SLOT_ID_1`/`HSM_SLOT_ID_2` so the server-crate multi-HSM routing test runs in CI. ([#942](https://github.com/Cosmian/kms/pull/942))
...
#### HSM key creation — sensitive by default

- **`HsmStore` defaults to `sensitive=true`**: when creating an HSM key (AES or RSA) without an explicit `sensitive` attribute, the PKCS#11 `CKA_SENSITIVE` flag is now always set to `true`. Previously `sensitive` defaulted to `false` (extractable), meaning keys created without `--sensitive` could be read out of the HSM. Pass `--sensitive false` explicitly only if an extractable key is required. ([#942](https://github.com/Cosmian/kms/pull/942))
- **Documentation**: `hsm_operations.md` examples updated — `--sensitive` flag removed from `ckms` create commands and a note added that sensitive is the default.
...
- **`/access/owned` visibility filtering**: `HsmStore::find()` now returns an empty list when `user_must_be_owner=true` and the caller is not an HSM admin, preventing HSM key leakage through the `/access/owned` REST endpoint. ([#942](https://github.com/Cosmian/kms/pull/942))
...
- **`/access/obtained` — HSM keys visible**: the SQL query for "obtained" access rights used `INNER JOIN objects` which excluded HSM keys (not stored in the `objects` table). Changed to `LEFT JOIN` with `COALESCE` so HSM key grants appear in obtained-access results with state defaulting to `Active`. ([#942](https://github.com/Cosmian/kms/pull/942))
- **`GetAttributes` allowed for any granted user**: `user_has_permission()` now allows `GetAttributes` (metadata-only, no key material) for any user who has at least one granted operation on the object. Previously, a user granted only `Encrypt` could Locate an object but not read its attributes, resulting in "Unknown" state in the UI. ([#942](https://github.com/Cosmian/kms/pull/942))
