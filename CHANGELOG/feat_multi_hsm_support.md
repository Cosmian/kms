...
- **`crate/server/src/tests/hsm/multi_hsm.rs`**: updated `test_multi_hsm_routing` — slot IDs now read from `HSM_SLOT_ID_1`/`HSM_SLOT_ID_2` env vars (were hardcoded 0/1), UID format updated to model-based scheme (`hsm::softhsm2::…`, `hsm::softhsm2_1::…`); test now pinned by `test_hsm_softhsm2.sh`. ([#942](https://github.com/Cosmian/kms/pull/942))
