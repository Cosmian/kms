...
> **`role_id` is not related to admin roles.**
> "Admin" in the Cosmian Authentication Server refers to human administrator accounts
> (the `Admin` struct, gated by the `_` realm).
> AppRoles are a completely separate, machine-identity concept.

| Credential | What it is | Analogy | Stability |
|---|---|---|---|
| `role_id` | A stable UUID auto-generated by auth-verifier when the AppRole is created. | *Username* for the machine identity. | Permanent — never changes. Rotate with care; SPIRE must be reconfigured if it changes. |
| `secret_id` | A short-lived, single-issue (or limited-use) secret tied to a role. | *Password* for the machine identity. | Ephemeral — can be rotated at any time via `generate-secret-id`; the old one is invalidated via its `secret_id_accessor`. |
| `secret_id_accessor` | An opaque UUID returned alongside `secret_id`. | *Revocation handle* — used to call `destroy-secret-id`. | Never sent to SPIRE; kept by the admin only. |

**How credentials become a KMS user identity**

After a successful `POST /v1/auth/approle/login`, auth-verifier issues a `hvs.*` token
whose internal `entity` field is set to the **AppRole name** (not the `role_id` UUID).
When the KMS receives a request carrying that token, the
`spire_token_middleware` validates it with a `GET /auth/token/lookup-self` call,
reads `data.entity_id` from the response, and sets:

```
AuthenticatedUser { username: entity_id }
```

This `username` becomes the **KMS object owner** for every operation performed under
that token:

| AppRole name | KMS `username` | Owns |
|---|---|---|
| `spire-server` | `spire-server` | PKI signing requests (reads CA key as server admin; signed cert stored under its identity) |
| `mistral-agents` | `mistral-agents` | Transit key pairs tagged `vault_transit:{name}` |

Two AppRoles therefore have **fully isolated KMS object namespaces**: the
`mistral-agents` role cannot read, sign with, or delete keys owned by `spire-server`,
and vice versa.
