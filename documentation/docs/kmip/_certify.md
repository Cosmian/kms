...
Specify X509 extensions for a `Certify` operation is possible using the [Cosmian CLI](../kms_clients/index.md).
...
Example of the corresponding [Cosmian CLI](../kms_clients/index.md) command:
...
The corresponding [Cosmian CLI](../kms_clients/index.md) command is:

```shell
cosmian kms certificates certify -r my_cert.csr -k 854d7914-3b1d-461a-a2dd-7aad27043b56 -d 365 -t "MyCert"
```

Note: the [Cosmian CLI](../kms_clients/index.md)ent converts the CSR from PEM TO DER before creating the JSON TTLV and sending
it to the
server.
