...
| `error` | `C_GetFunctionList: KmsClient::new_with_config panicked: {}`                                                                                                                                                                                                             | `src/lib.rs`                | `message`: panic payload from KmsClient::new_with_config                                                                                                       | PKCS#11 module failed to initialize the KMS client. Check the ckms.toml path and KMS configuration; the function returns CKR_FUNCTION_FAILED.                 |
...
| `info` | `C_GetFunctionList: begin (pp_function_list_null={})` | `src/lib.rs` | `pp_function_list_null` — whether the caller passed a null output pointer | Temporary diagnostic for the ASE PKCS#11 CI failure; remove once resolved |
| `info` | `C_GetFunctionList: returning CKR_OK` | `src/lib.rs` | — | Temporary diagnostic for the ASE PKCS#11 CI failure; remove once resolved |
...
| `info` | `C_Initialize: begin (args_null={})` | `src/pkcs11.rs` | `args_null` — whether `pInitArgs` was null | Temporary diagnostic for the ASE PKCS#11 CI failure; remove once resolved |
| `info` | `C_Initialize: flags=0x{:x}, reserved_null={}` | `src/pkcs11.rs` | `flags` — `CK_C_INITIALIZE_ARGS.flags`; `reserved_null` — whether `pReserved` was null | Temporary diagnostic for the ASE PKCS#11 CI failure; remove once resolved |
| `info` | `C_Initialize: returning CKR_OK` | `src/pkcs11.rs` | — | Temporary diagnostic for the ASE PKCS#11 CI failure; remove once resolved |
