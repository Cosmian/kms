...
| `debug` | `AuditFileStore: resuming at id={next_id}, prev_hash={}` | `src/core/audit/recovery.rs` | `next_id`, `prev_hash` | `prev_hash` is truncated to its first 8 bytes (hex) |
...
| `error` | `AuditFileStore: audit sink failed to resume ({e}) — audit logging is disabled for this process` | `src/core/audit/store.rs` | `e`: the resume error | Only reachable for the file backend, which never fails startup — a resume error here means every configured recovery path was exhausted; audit logging silently stops rather than crashing the server. |
| `error` | `AuditFileStore: failed to write recovery sentinel id={}: {e} — event dropped` | `src/core/audit/file_sink.rs` | `id`: the sentinel's assigned row id<br>`e`: the write error | Covers both `audit:torn-write-recovered` and `audit:reanchor` sentinel writes. |
| `debug` | `AuditFileStore: sink '{}' is at capacity — event dropped` | `src/core/audit/writer.rs` | sink name (`"file"` or `"postgres"`) | Logged when `AuditSink::is_write_capacity_exceeded()` reports the file backend's `max_size_bytes` cap has been reached; rate-limited to avoid flooding the log. |
...
| `error` | `audit: dedicated advisory-lock session ended unexpectedly: {e}` | `src/stores/audit/pgsql.rs` | `e`: the connection error | ×2 in this file — the dedicated session backing either the sink's own advisory-lock connection or a reader's setup connection. Losing this session releases the advisory lock; see ADR-0006's PostgreSQL concurrency notes. |
| `error` | `audit: instance_id={} sealed generation {sealed_generation} (reason={}, first_failure_id={}, evidence={evidence}) — starting generation {new_generation}` | `src/stores/audit/pgsql.rs` | `instance_id`: the audit chain this recovery applies to<br>`sealed_generation`: the generation left unmodified as forensic evidence<br>`reason`: one of `hash_mismatch`, `broken_link`, `unparseable`, `id_overflow`<br>`first_failure_id`: id of the first row that failed verification<br>`evidence`: `v1:sha256:<hex>` digest of the sealed generation, independently reproducible via `psql \| sha256sum` — see the audit operator guide<br>`new_generation`: the freshly started generation the KMS resumed into | Emitted once, after the `audit:reanchor` row commits — see ADR-0006 "PostgreSQL backend: seal-and-roll via immutable generations". |
