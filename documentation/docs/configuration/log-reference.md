...
| `debug` | `AuditFileStore: resuming at id={next_id}, prev_hash={}` | `src/core/audit/file_sink.rs` | `next_id`, `prev_hash` | `prev_hash` is truncated to its first 8 bytes (hex) |
| `debug` | `AuditFileStore: writer loop exited (channel closed)` | `src/core/audit/writer.rs` | - | Graceful shutdown complete |
...
| `error` | `AuditFileStore: cannot stat audit log {} ({e})` | `src/core/audit/file_sink.rs` | `e`: I/O error from `File::metadata()` | Only possible if `max_size_bytes` is configured; the cap check for this write is skipped (not treated as capped) and retried on the next write. |
...
| `error` | `AuditFileStore: audit sink failed to resume ({e}) — audit logging is disabled for this process` | `src/core/audit/store.rs` | `e`: error returned by the sink's `resume()` | File backend never reaches this in practice (its own `resume()` retries forever); a future backend whose `resume()` can genuinely fail leaves audit logging disabled for the process rather than aborting startup here — Postgres instead aborts startup earlier, in `AuditStore::start_postgres`. |
| `error` | `AuditFileStore: failed to write recovery sentinel id={}: {e} — event dropped` | `src/core/audit/file_sink.rs` | `id`: the sentinel event's assigned id<br>`e`: I/O error from the write | Torn-write-recovered / reanchor sentinel write failed during startup recovery; the chain resumes without advancing past this slot (same "never advance on failure" rule as steady-state writes). |
| `debug` | `AuditFileStore: sink '{}' is at capacity — event dropped` | `src/core/audit/writer.rs` | sink name (`"file"`/`"postgres"`) | Generic, backend-agnostic message: only the file backend ever reports capacity (its `max_size_bytes` cap); rate-limited to at most once every 500ms while blocked events keep arriving. |
...
| `error` | `audit: dedicated advisory-lock session ended unexpectedly: {e}` | `src/stores/audit/pgsql.rs` | `e`: connection driver error | The dedicated, unpooled session holding the instance's `PostgreSQL` advisory lock died (e.g. network partition) — the lock is released by `PostgreSQL` and a competing writer could take over the `instance_id` until this process reconnects. Composite-key defense-in-depth (see module docs) is what makes this window safe. ×2 in this file (one per TLS/no-TLS connection branch). |
