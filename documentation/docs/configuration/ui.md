...
When the KMS is deployed alongside a [Cosmian authentication server](https://github.com/Cosmian/authentication) ("AS"), the UI can offer a native username/password (with optional TOTP) login form that authenticates end users directly against it — the same capability as `ckms login cosmian`, exposed in the browser.
