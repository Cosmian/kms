...
PostgreSQL deployments used in multi-region active-active topologies additionally install an automatic state-monotonic-merge trigger on `objects` (see [Multi-region active-active PostgreSQL](./configuration.md#multi-region-active-active-postgresql-spock-pgedge-bdr)).
...
In PostgreSQL and MySQL, it is declared as the composite `PRIMARY KEY (id, userid)` (providing valid `REPLICA IDENTITY` for logical replication); in SQLite it is declared `UNIQUE (id, userid)`.
...
In PostgreSQL and MySQL, it is declared as the composite `PRIMARY KEY (id, tag)` (providing valid `REPLICA IDENTITY` for logical replication); in SQLite it is declared `UNIQUE (id, tag)`.
...
In PostgreSQL, an `id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY` column is added to provide valid `REPLICA IDENTITY` for logical replication.
In SQLite there is no explicit `id` column; the active activation is the latest row where `revoked_at IS NULL`.
