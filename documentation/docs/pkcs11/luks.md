...
1. Create a LUKS partition on the new partition.
Enter a password to protect the partition when prompted.
```bash
sudo cryptsetup luksFormat /dev/vda4
```

1. Open the LUKS partition and map it to a device `/dev/mapper/luks`.
Enter the password when prompted.
```bash
sudo cryptsetup luksOpen /dev/vda4 luks
```

1. Format the mapped device with a filesystem of your choice.
```bash
...
The Cosmian KMS provides a PKCS#11 module that can be used to access the KMS from applications that support PKCS#11,
using the `p11-kit` framework. Unfortunately, the support for p11-kit in Ubuntu 20.04 is not complete,
