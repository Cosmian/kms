...
<div style="font-size:2rem;font-weight:800;color:#7c3aed">57%</div>
...
<div style="font-size:2rem;font-weight:800;color:#6b7280">46%</div>
...
=== "📈 Risk Summary"
    ...
    | 🔵 LOW / ⚪ INFO | **2334** | Informational algorithm usage references |
...
=== "🔬 Algorithm Profile"
    ...
    | RSA | Asymmetric | ✅ | ❌ | 246 |
    | Covercrypt (ABE) | Attribute-Based Encryption | ❌ | — | 241 |
    | ML-KEM (FIPS 203) | Post-Quantum KEM | ✅ | ✅ | 222 |
    | X.509 certificate | PKI / TLS | ✅ | — | 213 |
    | SLH-DSA (FIPS 205) | Post-Quantum Signature | ✅ | — | 207 |
    | EdDSA (Ed25519/Ed448) | Asymmetric | ✅ | ❌ | 107 |
    | AES-GCM/GCM-SIV | Symmetric | ✅ | — | 93 |
    | ML-DSA (FIPS 204) | Post-Quantum Signature | ✅ | — | 69 |
...
=== "📦 Dependencies"
    ...
    |------------|-------------|----------|----------------:|
    | `openssl (FIPS provider)` | openssl (FIPS provider) |  | 77 |
    ...
    flowchart TD
        _["."]
        _ --> OPENSSL__FIPS_PROVIDER_["openssl (FIPS provider)"]
        _ --> OPENSSL["OpenSSL (FIPS provider)"]
        _ --> X509_PARSER["x509-parser"]
        _ --> COSMIAN_CRYPTO_CORE["cosmian_crypto_core (KEM combiner)"]
        _ --> P256["p256 NIST P-256"]
        _ --> AES_GCM["RustCrypto/aes-gcm"]
        _ --> ARGON2["RustCrypto/argon2"]
        _ --> COSMIAN_COVER_CRYPT["cosmian_cover_crypt (ABE)"]
        _ --> K256["k256 secp256k1"]
        _ --> RUSTLS["rustls (TLS)"]
        _ --> RING["ring (BoringSSL subset)"]
    ```
...
**Score: 57%** — 57% of asymmetric operations have a PQC alternative.

```mermaid
pie title PQC vs Classical asymmetric coverage
    "PQC-ready (ML-KEM, ML-DSA, SLH-DSA, Hybrid)" : 57
    "Classical-only (RSA, EC, EdDSA)" : 43
```
...
**Score: 46%** of detected algorithm references are FIPS 140-3 approved.

The remaining 54% are:
