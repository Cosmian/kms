...
Options:
  -c, --config <COSMIAN_KMS_CONF>
      ...
      --max-connections <MAX_CONNECTIONS>
          Maximum number of connections for the relational database pool. When not provided, falls back to the current defaults: - `PostgreSQL`/`MySQL`: 5 - `SQLite`: number of CPUs [env: KMS_DB_MAX_CONNECTIONS=]
