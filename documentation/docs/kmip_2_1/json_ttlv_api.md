The easiest way to call the KMIP API is to use the `cosmian` [Command Line Interface](/cosmian_cli)
...
!!!info  "Easily building JSON TTLV messages"
    ...
    the [Cosmian CLI](/cosmian_cli) in `JSON mode` mode to print the corresponding request and response messages.
