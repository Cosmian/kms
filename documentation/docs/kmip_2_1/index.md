...
1. When `Attributes` are passed as part of the KMIP operation, such as in the `Create`, `Create Key Pair`, `Locate`, and `Import` operations, 
...
2. When unique identifiers are passed as part of the KMIP operation, such as in the `Certify`, `Encrypt`, `Decrypt`, `Get`, `Get Attributes`, `Revoke`, and `Destroy` operations,
...
Although querying the KMS server is normally done using the [ckms](../cli/cli.md) command line interface or one of the [Cloudproof libraries](https://docs.cosmian.com/cloudproof_encryption/application_level_encryption/),
direct REST POST requests can be made to the KMS server using the JSON TTLV format.
