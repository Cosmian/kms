...
test.describe("Keyset addressing syntax", () => {
    test("encrypt with bare keyset name resolves to latest key", async ({ page }) => {
        ...
        await setRotationPolicy(page, keyId, "e2e-ks-bare", 86400);

        const plainFile = writeTempFile("ks-bare.txt", PLAINTEXT);
        const encPath = await symEncrypt(page, "e2e-ks-bare", plainFile);
    ...
    test("encrypt with name@latest resolves to latest key", async ({ page }) => {
        const keyId = await createSymKey(page);
        await setRotationPolicy(page, keyId, "e2e-ks-latest", 86400);

        const plainFile = writeTempFile("ks-latest.txt", PLAINTEXT);
        const encPath = await symEncrypt(page, "e2e-ks-latest@latest", plainFile);

        const decPath = await symDecrypt(page, keyId, encPath);
        expect(fs.readFileSync(decPath, "utf-8")).toBe(PLAINTEXT);
    });

    test("encrypt with name@first after rekey resolves to gen-0", async ({ page }) => {
        const keyId = await createSymKey(page);
        await setRotationPolicy(page, keyId, "e2e-ks-first");

        // Rotate: gen-0 → gen-1
        await rekeyKey(page, keyId);

        // Encrypt with @first — must use gen-0
        const plainFile = writeTempFile("ks-first.txt", PLAINTEXT);
        const encPath = await symEncrypt(page, "e2e-ks-first@first", plainFile);

        // Decrypt with gen-0 UUID proves @first resolved correctly
        const decPath = await symDecrypt(page, keyId, encPath);
        expect(fs.readFileSync(decPath, "utf-8")).toBe(PLAINTEXT);
    });

    test("encrypt with name@0 after rekey resolves to gen-0", async ({ page }) => {
        const keyId = await createSymKey(page);
        await setRotationPolicy(page, keyId, "e2e-ks-zero");

        await rekeyKey(page, keyId);

        const plainFile = writeTempFile("ks-zero.txt", PLAINTEXT);
        const encPath = await symEncrypt(page, "e2e-ks-zero@0", plainFile);

        const decPath = await symDecrypt(page, keyId, encPath);
        expect(fs.readFileSync(decPath, "utf-8")).toBe(PLAINTEXT);
    });

    test("encrypt with name@1 after double rekey resolves to gen-1", async ({ page }) => {
        const keyId = await createSymKey(page);
        await setRotationPolicy(page, keyId, "e2e-ks-gen1");

        const gen1Id = await rekeyKey(page, keyId);
        await rekeyKey(page, gen1Id);

        // Encrypt with @1 — must use gen-1
        const plainFile = writeTempFile("ks-gen1.txt", PLAINTEXT);
        const encPath = await symEncrypt(page, "e2e-ks-gen1@1", plainFile);

        // Decrypt with gen-1 UUID proves @1 resolved correctly
        const decPath = await symDecrypt(page, gen1Id, encPath);
        expect(fs.readFileSync(decPath, "utf-8")).toBe(PLAINTEXT);
    });

    test("decrypt with bare keyset name walks chain after rotation", async ({ page }) => {
        const keyId = await createSymKey(page);
        await setRotationPolicy(page, keyId, "e2e-ks-chain");
        ...
        const decPath = await symDecrypt(page, "e2e-ks-chain", encPath);
        expect(fs.readFileSync(decPath, "utf-8")).toBe(PLAINTEXT);
    });

    test("encrypt with name@99 fails for nonexistent generation", async ({ page }) => {
        const keyId = await createSymKey(page);
        await setRotationPolicy(page, keyId, "e2e-ks-bad");

        const plainFile = writeTempFile("ks-bad.txt", PLAINTEXT);
        await gotoAndWait(page, "/ui/sym/encrypt");
        await uploadFile(page, plainFile);
        await page.fill('input[placeholder="Enter key ID"]', "e2e-ks-bad@99");
        const text = await submitAndWaitForResponse(page);
    ...
    test("get rotation policy after rekey shows incremented generation", async ({ page }) => {
        const keyId = await createSymKey(page);
        await setRotationPolicy(page, keyId, "e2e-ks-gen", 86400);
