...
async function waitForKmsServer(): Promise<void> {
    ...
    throw new Error(
        `KMS server not reachable at ${KMS_URL} within 60s. ` +
            `Start it with: docker compose -f ui/docker-compose.yml up -d. Last error: ${String(lastError)}`
    );
}
...
describe.sequential("KMS secret data export/import roundtrip", () => {
    beforeAll(async () => {
    ...
    test("secret data: create (from value) → locate → export json-ttlv → import under new ID → locate → cleanup", async () => {
        ...
        const createReq = wasm.create_secret_data_ttlv_request("Password", secretValue, undefined, baseTags, false, undefined);
