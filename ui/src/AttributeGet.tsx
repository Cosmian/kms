...
const LINK_TYPES = [
  { value: 'Certificate', label: 'Certificate',
    description: 'For Certificate objects: the parent certificate for a certificate in a certificate chain. For Public Key objects: the corresponding certificate(s), containing the same public key.' },
  { value: 'PublicKey', label: 'Public Key',
    description: 'For a Private Key object: the public key corresponding to the private key. For a Certificate object: the public key contained in the certificate.' },
  { value: 'PrivateKey', label: 'Private Key',
    description: 'For a Public Key object: the private key corresponding to the public key.' },
  { value: 'DerivationBaseObject', label: 'Derivation Base Object',
  ...
  { value: 'DerivedKey', label: 'Derived Key',
    description: 'The symmetric key(s) or Secret Data object(s) that were derived from the current object.' },
  { value: 'ReplacementObject', label: 'Replacement Object',
    description: 'For a Symmetric Key, an Asymmetric Private Key, or an Asymmetric Public Key object: the key that resulted from the re-key of the current key.' },
  { value: 'ReplacedObject', label: 'Replaced Object',
    description: 'For a Symmetric Key, an Asymmetric Private Key, or an Asymmetric Public Key object: the key that was re-keyed to obtain the current key.' },
  ...
  { value: 'Previous', label: 'Previous',
    description: 'For all object types: the previous object to this object.' },
  { value: 'Next', label: 'Next',
    description: 'For all object types: the next object to this object.' },
  { value: 'PKCS12Certificate', label: 'PKCS12 Certificate' },
  { value: 'PKCS12Password', label: 'PKCS12 Password' },
  { value: 'WrappingKey', label: 'Wrapping Key',
    description: 'For wrapped objects: the object that was used to wrap this object.' }
];
  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-8 space-y-2">
        <div>
          Retrieve attributes and tags for a KMIP object by specifying either the object ID or tags.
        </div>
        <div className="text-sm text-yellow-600">
          When using tags to retrieve the object, rather than the object id, an error is returned if multiple objects matching the tags are found.
        </div>
      </div>
      ...
      >
        <Space direction="vertical" size="middle" style={{ display: 'flex' }}>
          <Card title="Object Identification">
            <div className='mb-5'>
              Specify either the Object ID or one or more tags to identify the object.
            </div>

            <Form.Item
              name="id"
              label="Object ID"
              help="The unique identifier of the cryptographic object"
            >
            ...
            <Form.Item
              name="tags"
              label="Tags"
              help="Tags to use to retrieve the key when no key ID is specified"
            >
              <Select
                mode="tags"
                style={{ width: '100%' }}
          ...
          <Card title="Attribute Selection">
            ...
            >
              <Select
                mode="multiple"
                style={{ width: '100%' }}
                placeholder="Select attribute tags"
              >
                {KMIP_TAGS.map(tag => (
