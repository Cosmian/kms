...
const HashMapDisplay: React.FC<HashMapDisplayProps> = ({ data }) => {
    ...
    const formatDisplayKey = (key: any): React.ReactNode => {
        if (typeof key === "string") return <span className="text-yellow-600 dark:text-yellow-400">"{key}"</span>;
        if (typeof key === "number") return <span className="text-orange-600 dark:text-orange-400">{key}</span>;
        return <span className="text-gray-800 dark:text-gray-200">{String(key)}</span>;
    };
    ...
    const renderValueWithColor = (value: any): React.ReactNode => {
        ...
        if (value === undefined) return <span className="text-gray-600 dark:text-gray-300">undefined</span>;
        ...
        if (typeof value === "object") {
            return <span className="text-blue-600 dark:text-blue-400">{JSON.stringify(value, null, 2)}</span>;
        }
    ...
    const renderMapPreview = (map: Map<any, any>): React.ReactNode => {
        ...
        return (
            <div className="p-2 rounded-md overflow-auto text-sm font-mono border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
