...
PLATFORM_SPECIFIC_CRATES: frozenset[str] = frozenset(
    {'crate/clients/cng'}
)  # Windows-only
...
def _orange(s):
    return f"{ORANGE}{s}{RESET}" if _USE_COLOR else s


def _green(s):
    return f"{GREEN}{s}{RESET}" if _USE_COLOR else s


def _bold(s):
    return f"{BOLD}{s}{RESET}" if _USE_COLOR else s


def _dim(s):
    return f"{DIM}{s}{RESET}" if _USE_COLOR else s
...
class MergeResult:
    ...
    )  # (DocEntry, old_mult: int, new_mult: int)
...
def parse_doc(path: Path) -> tuple[list[str], list[DocEntry], dict[str, int]]:
    ...
    for i, line in enumerate(lines):
        ...
        if (
            current_crate
            and _TABLE_SEP_RE.match(line)
            and current_crate not in crate_sep_idx
        ):
        ...
        while j < len(body):
            if body[j] == '\\':
                ...
                if nxt == '\n':
                    ...
                    chars.append('\\')
                    chars.append('t')  # \t → keep as literal \t
                elif nxt == '"':
                    chars.append('"')  # \" → " (unescape)
                elif nxt == '\\':
                    chars.append('\\')  # \\ → \ (unescape)
                else:
                    chars.append(body[j : j + 2])  # keep raw
                j += 2
                continue
            if body[j] == '"':
                break
...
def merge(
    ...
    for e in doc_entries:
        ...
        if qk in quad_set:
            ...
            if (
                e.crate_path in PLATFORM_SPECIFIC_CRATES
                and not (REPO_ROOT / e.crate_path).exists()
            ):
                continue
            if silent_delete:
                result.deleted.append(e)
...
def rewrite_doc(
    ...
    for cp, rows in new_by_crate.items():
        ...
        if insert_after is not None:
            new_rows = [
                _build_new_row(cp, f, lv, msg, mult) for (lv, msg, f, mult) in rows
            ]
    ...
    for i, line in enumerate(lines):
        ...
        if i in mult_upd_map:
            ...
            if len(parts) >= 7:
                parts[5] = (
                    ' ' + _update_mult_in_notes(parts[5].strip(), mult_upd_map[i]) + ' '
                )
                line = '|'.join(parts)
...
def _print_report(result: MergeResult, version: str, silent_delete: bool) -> None:
    ...
    parts: list[str] = []
    if stale_all:
        parts.append(_red(f"{len(stale_all)} stale"))
    if result.moved:
        parts.append(_orange(f"{len(result.moved)} moved"))
    if result.mult_upd:
        parts.append(_orange(f"{len(result.mult_upd)} ×N updated"))
    if result.added:
        parts.append(_green(f"{len(result.added)} new"))
    if parts:
        print('  ' + ' · '.join(parts))
    else:
...
def main() -> None:
    ...
    if args.non_interactive:
        ...
        if args.delete_stale:
            print(
                _orange(
                    '  ⚠  --delete-stale is only used with --non-interactive; ignoring.'
                )
            )
        silent_delete = _phase0_prompts()
    print(
        _dim(
            f"  Version: {version}  |  Stale strategy: {'delete' if silent_delete else 'flag [REMOVED]'}"
        )
    )
    print()
