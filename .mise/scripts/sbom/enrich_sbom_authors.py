...
def _read_cargo_toml_from_cache(
...
def _api_fetch_crate_info(name: str, api_cache: dict[str, dict]) -> dict[str, str]:
    ...
    try:
        ...
        names = [o.get("name") or o.get("login") or "" for o in owners if o.get("name") or o.get("login")]
...
def collect_rust_crate_metadata(
    ...
    if api_calls > 0:
        ...
        print(f"  → {api_calls} crates.io API calls made, cache saved to {API_CACHE_PATH}")
...
def collect_npm_metadata(repo_root: Path, api_limit: int = 0) -> list[dict]:
    ...
    if not lock_path.exists():
        print("  ⚠  ui/pnpm-lock.yaml not found, skipping npm enrichment", file=sys.stderr)
        return []
    ...
    if api_calls > 0:
        API_CACHE_PATH.write_text(json.dumps(api_cache, indent=2))
        print(f"  → {api_calls} npm registry API calls made, cache saved to {API_CACHE_PATH}")
...
def enrich_spdx(
    ...
    for idx, comp in enumerate(rust_components + npm_components):
        ...
        if key not in existing:
            ...
            for pkg in bom.get("packages", []):
                if pkg.get("name") == comp["name"] and pkg.get("versionInfo") == comp["version"]:
    ...
    root_pkg = next(
        (p["SPDXID"] for p in bom.get("packages", []) if "cosmian" in p.get("name", "").lower()),
        doc_id,
    )
    new_rels = [
        {
            "spdxElementId": root_pkg,
            "relationshipType": "DYNAMIC_LINK" if comp["ecosystem"] == "npm" else "STATIC_LINK",
...
def main() -> None:
    ...
    sbom_dir = Path(args.sbom_dir) if Path(args.sbom_dir).is_absolute() else repo_root / args.sbom_dir
    ...
    if not args.no_rust:
        ...
        rust_components = collect_rust_crate_metadata(repo_root, api_limit=args.api_limit)
        print()
    ...
    if cdx_path.exists():
        ...
        enrich_cyclonedx(cdx_path, rust_components, npm_components, in_place=args.in_place)
    else:
        print(f"  ⚠  {cdx_path} not found, skipping CycloneDX enrichment", file=sys.stderr)
