...
require_cmd python3 "Python 3 is required. Re-run via 'bash .mise/scripts/nix.sh test pykmip' so nix-shell can provide it."
...
env -u LD_LIBRARY_PATH -u OPENSSL_CONF -u OPENSSL_MODULES bash "$REPO_ROOT/.mise/scripts/pykmip/test_pykmip.sh" all
PYKMIP_STATUS=$?
