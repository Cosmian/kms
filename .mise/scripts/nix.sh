...
usage() {
  ...
  Commands:
    ...
    test [type] [args] Run tests inside nix-shell
      ...
      cef                    Run CEF audit export test (live server + jc)
...
docker_command() {
  ...
  VERSION=$(bash "$REPO_ROOT/.mise/scripts/release/get_version.sh")
  ...
  if [ "$DOCKER_LOAD" = true ]; then
    if command -v docker >/dev/null 2>&1; then
      ...
      if [ "$DOCKER_TEST" = true ]; then
        ...
        bash "$REPO_ROOT/.mise/scripts/test/test_docker_image.sh"
      fi
    else
...
test_command() {
  case "$TEST_TYPE" in
    all)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_all.sh"
      ;;
    aws_xks)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_xks.sh"
      ;;
    wasm)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_wasm.sh"
      ;;
    sqlite)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_sqlite.sh"
      ;;
    mysql)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_mysql.sh"
      ;;
    percona)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_percona.sh"
      ;;
    otel_export)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_otel_export.sh"
      ;;
    mariadb)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_maria.sh"
      ;;
    psql)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_psql.sh"
      ;;
    redis)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_redis.sh"
      ;;
    azure_ekm)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_azure_ekm.sh"
      ;;
    jose)
    ...
    iris)
      ...
      ;;
    cef)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_cef.sh"
      ;;
    gcp_cmek)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_gcp_cmek.sh"
      ;;
    google_cse)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_google_cse.sh"
      ...
      ;;
    pykmip)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_pykmip.sh"
      ;;
    edb_tde)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_edb_tde.sh"
      ;;
    openssh)
      SCRIPT="$REPO_ROOT/.mise/scripts/test/test_openssh.sh"
      ;;
    luks)
    ...
    hsm)
      ...
      case "$HSM_BACKEND" in
        all)
          SCRIPT="$REPO_ROOT/.mise/scripts/test/test_hsm.sh"
          ;;
        softhsm2)
          SCRIPT="$REPO_ROOT/.mise/scripts/test/test_hsm_softhsm2.sh"
          shift
          ;;
        utimaco)
          SCRIPT="$REPO_ROOT/.mise/scripts/test/test_hsm_utimaco.sh"
          shift
          ;;
        proteccio)
          SCRIPT="$REPO_ROOT/.mise/scripts/test/test_hsm_proteccio.sh"
          shift
          ;;
        crypt2pay)
          SCRIPT="$REPO_ROOT/.mise/scripts/test/test_hsm_crypt2pay.sh"
          shift
          ;;
        *)
    ...
    *)
      ...
      echo "Valid types: aws_xks, sqlite, mysql, percona, mariadb, psql, redis, google_cse, gcp_cmek, pykmip, openssh, luks, otel_export, iris, jose, cef, hsm [softhsm2|utimaco|proteccio|all], ui, secret_vault, secret_aws, secret_azure, secret_cosmian_kms" >&2
      usage
      ;;
  esac
  ...
  if [ "$TEST_TYPE" = "pykmip" ] || [ "$TEST_TYPE" = "jose" ] || [ "$TEST_TYPE" = "edb_tde" ] || [ "$TEST_TYPE" = "cef" ]; then
...
sbom_command() {
  ...
  SCRIPT="$REPO_ROOT/.mise/scripts/sbom/generate_sbom.sh"
...
update_hashes_command() {
  SCRIPT="$REPO_ROOT/.mise/scripts/release/update_hashes.sh"
  [ -f "$SCRIPT" ] || {
...
package_command() {
  ...
  if [ "$(uname)" = "Darwin" ]; then
    ...
    if [ "$PACKAGE_TYPE" = "dmg" ]; then
      SCRIPT="$REPO_ROOT/.mise/scripts/package/package_dmg.sh"
      KEEP_VARS=""
    ...
    for BUILD_LINK in "${LINKS_TO_BUILD[@]}"; do
      ...
      for TYPE in $TYPES; do
        case "$TYPE" in
          deb)
            if [ "$(uname)" = "Linux" ]; then
              SCRIPT_LINUX="$REPO_ROOT/.mise/scripts/package/package_deb.sh"
              [ -f "$SCRIPT_LINUX" ] || {
              ...
              if [ -n "$DEB_FILE" ] && [ -f "$DEB_FILE" ]; then
                SMOKE_TEST_SCRIPT="$REPO_ROOT/.mise/scripts/package/smoke_test_deb.sh"
              ...
              if [ -n "$CLI_DEB_FILE" ] && [ -f "$CLI_DEB_FILE" ]; then
                CLI_SMOKE_TEST_SCRIPT="$REPO_ROOT/.mise/scripts/package/smoke_test_cli_deb.sh"
          ...
          rpm)
            if [ "$(uname)" = "Linux" ]; then
              SCRIPT_LINUX="$REPO_ROOT/.mise/scripts/package/package_rpm.sh"
              [ -f "$SCRIPT_LINUX" ] || {
                echo "Missing $SCRIPT_LINUX" >&2
                exit 1
              }
              ...
              if [ -n "$RPM_FILE" ] && [ -f "$RPM_FILE" ]; then
                SMOKE_TEST_SCRIPT="$REPO_ROOT/.mise/scripts/package/smoke_test_rpm.sh"
                if [ -f "$SMOKE_TEST_SCRIPT" ]; then
              ...
              if [ -n "$CLI_RPM_FILE" ] && [ -f "$CLI_RPM_FILE" ]; then
                CLI_SMOKE_TEST_SCRIPT="$REPO_ROOT/.mise/scripts/package/smoke_test_cli_rpm.sh"
                if [ -f "$CLI_SMOKE_TEST_SCRIPT" ]; then
          ...
          dmg)
            ...
            env -u LD_LIBRARY_PATH -u LD_PRELOAD nix-build -I "nixpkgs=${NIXPKGS_ARG}" "$REPO_ROOT/default.nix" -A "$ATTR" -o "$OUT_LINK"
            REAL_OUT=$(readlink -f "$OUT_LINK" || echo "$OUT_LINK")
            echo "Built dmg ($BUILD_VARIANT-$BUILD_LINK): $REAL_OUT"

            DMG_FILE=$(find "$REAL_OUT" -maxdepth 1 -type f -name '*.dmg' | head -n1 || true)
            SMOKE_TEST_SCRIPT="$REPO_ROOT/.mise/scripts/package/smoke_test_dmg.sh"
            if [ -n "$DMG_FILE" ] && [ -f "$DMG_FILE" ]; then
              if [ -f "$SMOKE_TEST_SCRIPT" ]; then
          ...
          pkcs11-zip)
            if [ "$(uname)" = "Linux" ]; then
              PKCS11_ZIP_SCRIPT="$REPO_ROOT/.mise/scripts/package/package_pkcs11_zip.sh"
              [ -f "$PKCS11_ZIP_SCRIPT" ] || {
