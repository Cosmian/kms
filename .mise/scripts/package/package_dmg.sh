...
# Source sign_packages from package_common.sh via tmpfile (process substitution
# source <(...) is unreliable on macOS bash; tmpfile is portable and explicit).
COMMON_LIB="$SCRIPT_DIR/package_common.sh"
if [ -f "$COMMON_LIB" ]; then
  _sign_tmp=$(mktemp)
  sed -n '/^sign_packages()/,/^}/p' "$COMMON_LIB" >"$_sign_tmp"
