...
ensure_expected_hashes() {
  ...
  local server_vendor_file="$hashes_dir/server.vendor.${link_suffix}.sha256"
  [ -s "$server_vendor_file" ] || missing=1
  # CLI vendor cargo hash: Linux fips==non-fips → shared file; macOS fips≠non-fips → variant-specific
  local cli_vendor_file
  if [ "$os" = "linux" ]; then
    cli_vendor_file="$hashes_dir/cli.vendor.linux.sha256"
  else
    cli_vendor_file="$hashes_dir/cli.vendor.${VARIANT}.${os}.sha256"
  fi
  [ -s "$cli_vendor_file" ] || missing=1
...
prewarm_store() {
  ...
  [ $need_cli -eq 1 ] && nix-build -I "nixpkgs=${PIN_URL}" -A "$cli_attr" --no-out-link >/dev/null || echo "CLI derivation already present"
}
...
build_or_reuse_cli() {
  ...
  if [ -L "$CLI_OUT_LINK" ] && [ -x "$(readlink -f "$CLI_OUT_LINK")/bin/ckms" ]; then
    ...
    nix-build -I "nixpkgs=${PIN_URL}" --option substituters "" "$REPO_ROOT/default.nix" -A "$attr" -o "$CLI_OUT_LINK"
    REAL_CLI=$(readlink -f "$CLI_OUT_LINK" || echo "$CLI_OUT_LINK")
  fi
...
resolve_openssl_path() {
  ...
  if [ "$VARIANT" = "fips" ] && [ -d "$REAL_SERVER/usr/local/cosmian/lib/ssl" ]; then
    ...
    if [ -f "$conf" ]; then
      # Use portable tmpfile pattern — BSD sed (macOS) doesn't accept GNU-style sed -i without extension
      local _tmp
      _tmp=$(mktemp)
      sed 's|^\s*#\s*\.include.*fipsmodule\.cnf|.include /usr/local/cosmian/lib/ssl/fipsmodule.cnf|' "$conf" >"$_tmp" && mv "$_tmp" "$conf"
      if ! grep -q '^\.include /usr/local/cosmian/lib/ssl/fipsmodule.cnf' "$conf"; then
        # GNU sed '1i text' is not portable; prepend via printf+cat instead
        _tmp=$(mktemp)
        {
          printf '.include /usr/local/cosmian/lib/ssl/fipsmodule.cnf\n'
          cat "$conf"
        } >"$_tmp" && mv "$_tmp" "$conf"
      fi
    fi
