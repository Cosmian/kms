...
#
# Usage: bash .mise/scripts/audit/multi_framework.sh [--verbose]
#
# The script also writes a markdown report to:
#   documentation/docs/certifications_and_compliance/audit/multi_framework_security_audit.md
# =============================================================================
...
if [ "$UNSAFE_COUNT" -lt 350 ]; then
  _pass "unsafe block count: $UNSAFE_COUNT (< 350 threshold; mostly FFI wrappers)"
else
  _fail "unsafe block count: $UNSAFE_COUNT (≥ 350 — significant growth since last audit)"
fi
if [ -n "$VERBOSE" ]; then
...
REPORT_PATH="$REPO_ROOT/documentation/docs/certifications_and_compliance/audit/multi_framework_security_audit.md"
...
TIMESTAMP=$(date -u '+%Y-%m-%dT%H:%M:%SZ')
{
  ...
  echo "**Frameworks**: NIST CSF 2.0 / SSDF (SP 800-218) · CIS Controls v8 · ISO/IEC 27034 · OSSTMM"
  echo "**Repository**: \`$(basename "$REPO_ROOT")\` — branch \`$(git rev-parse --abbrev-ref HEAD 2>/dev/null || echo "unknown")\`"
  echo "**Commit**: \`$(git rev-parse --short HEAD 2>/dev/null || echo "unknown")\`"
  echo ""
  echo "## Automated Audit Checks"
  echo ""
  echo "| Status | Check |"
  echo "|--------|-------|"
  ...
  echo ""
  if [ "$FAIL" -gt 0 ]; then
    echo "> **AUDIT FAILED** — $FAIL check(s) must be addressed before release."
  elif [ "$WARN" -gt 0 ]; then
    echo "> **AUDIT PASSED WITH WARNINGS** — $WARN item(s) require review."
  else
    echo "> **AUDIT PASSED** — all checks cleared."
  fi
  echo ""
  echo "---"
  echo ""
  echo "*Report auto-generated by \`.mise/scripts/audit/multi_framework.sh\` on $TIMESTAMP*"
} >"$REPORT_PATH"
echo ""
echo -e "${GREEN}Report written to: ${REPORT_PATH}${NC}"
