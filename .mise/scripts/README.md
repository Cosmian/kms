...
- **Reproducible builds**: Pinned dependencies (nixpkgs 24.05, Rust 1.97.0, OpenSSL 3.6.2 + OpenSSL 3.1.2 FIPS provider)
...
```bash
bash .mise/scripts/nix.sh docker [--variant <fips|non-fips>] [--force] [--load] [--test]
```
...
bash .mise/scripts/nix.sh docker --variant non-fips --load

# Build, load and run container tests
bash .mise/scripts/nix.sh docker --variant fips --load --test
```
...
bash .mise/scripts/nix.sh [--variant <fips|non-fips>] [--link <static|dynamic>] test [type] [backend]
```
...
bash .mise/scripts/nix.sh test

# Specific database tests
bash .mise/scripts/nix.sh test sqlite
bash .mise/scripts/nix.sh test psql

# Percona / MariaDB
bash .mise/scripts/nix.sh test percona
bash .mise/scripts/nix.sh test mariadb

# Redis tests (non-FIPS required)
bash .mise/scripts/nix.sh --variant non-fips test redis

# PyKMIP client tests (non-FIPS, includes Python environment)
bash .mise/scripts/nix.sh --variant non-fips test pykmip

# OTEL export integration tests (requires Docker)
bash .mise/scripts/nix.sh test otel_export

# WASM tests
bash .mise/scripts/nix.sh test wasm
...
TEST_GOOGLE_OAUTH_REFRESH_TOKEN=... \
GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY=... \
  bash .mise/scripts/nix.sh test google_cse

# HSM tests (specific backend)
bash .mise/scripts/nix.sh test hsm softhsm2
bash .mise/scripts/nix.sh test hsm all
```
...
bash .mise/scripts/nix.sh package
...
bash .mise/scripts/nix.sh package deb
bash .mise/scripts/nix.sh package rpm

# Build non-FIPS variant
bash .mise/scripts/nix.sh --variant non-fips package deb
bash .mise/scripts/nix.sh --variant non-fips package dmg

# Build dynamic OpenSSL linkage (system OpenSSL; packaging still bundles needed libs)
bash .mise/scripts/nix.sh --link dynamic package deb
```
...
```bash
bash .mise/scripts/nix.sh [--variant <fips|non-fips>] [--link <static|dynamic>] sbom [--target <openssl|server>]
```
...
bash .mise/scripts/nix.sh sbom

# SBOM for KMS server (FIPS, static)
bash .mise/scripts/nix.sh sbom --target server

# SBOM for KMS server (non-FIPS, static)
bash .mise/scripts/nix.sh --variant non-fips --link static sbom --target server

# SBOM for KMS server (FIPS, dynamic)
bash .mise/scripts/nix.sh --variant fips --link dynamic sbom --target server
```
...
bash .mise/scripts/nix.sh update-hashes [RUN_ID]
```
...
bash .mise/scripts/nix.sh update-hashes

# Use a specific workflow run
bash .mise/scripts/nix.sh update-hashes 123456789
```
...
| **Reproducible Toolchain** | Rust 1.97.0 from Nix (no rustup)              | Eliminates "works on my machine" compiler issues |
...
2. **If CI failed on a fixed-output derivation hash** (Cargo vendor / UI deps):
   - Run `bash .mise/scripts/nix.sh update-hashes [RUN_ID]` to update `nix/expected-hashes/*` from CI logs
...
#### `.mise/scripts/` (root — entrypoints only)
...
flowchart TB
    ...
    gen[".mise/scripts/sbom/generate_sbom.sh"]
...
flowchart LR
    subgraph pure["PURE MODE (--pure flag)"]
        ...
        p_env["Rust 1.97.0 (Nix)<br/>OpenSSL 3.6.2 + 3.1.2 (FIPS)<br/>Build tools · /nix/store paths ONLY"]
    end
...
1. **Create test script: `.mise/scripts/test/test_<name>.sh``
...
2. **Update dispatcher**: Add case to `nix.sh` test command handling
   ...
   ```bash
   <name>)
     SCRIPT="$REPO_ROOT/.mise/scripts/test/test_<name>.sh"
...
```bash
# Development
bash .mise/scripts/nix.sh test sqlite                # Quick test iteration

# Build a package (this also builds the server)
bash .mise/scripts/nix.sh package deb

# Release preparation
bash .mise/scripts/nix.sh --variant fips test all
bash .mise/scripts/nix.sh --variant non-fips test all
bash .mise/scripts/nix.sh package                    # All packages
bash .mise/scripts/nix.sh sbom                       # OpenSSL 3.1.2 derivation SBOM
bash .mise/scripts/nix.sh sbom --target server       # Server SBOM (default fips/static)
bash .mise/scripts/nix.sh --variant non-fips sbom --target server

# Hash maintenance
bash .mise/scripts/nix.sh update-hashes                 # Update expected-hashes from latest CI logs
bash .mise/scripts/nix.sh update-hashes 123456789       # Use a specific workflow run

# CI simulation
NO_PREWARM=1 bash .mise/scripts/nix.sh package deb   # Skip prewarm (cached store)
```
