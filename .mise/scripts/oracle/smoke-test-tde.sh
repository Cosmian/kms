...
# Defined here so it can be reused in both CDB and PDB recovery paths.
...
# Recovery for PDB CLOSED (ORA-28353) when CDB is already OPEN:
# CDB was left open from a previous run but PDB has a stale ME ID no longer in KMS.
if echo "${pdb_status}" | grep -q "CLOSED"; then
  echo "WARNING: PDB wallet CLOSED (ORA-28353) while CDB is OPEN — importing placeholder PDB ME into KMS"
  pdb_me_raw=$(
    sudo -u oracle bash -s <<'ORACLEBASH'
export ORACLE_HOME=/opt/oracle/product/23ai/dbhomeFree
export ORACLE_SID=FREE
