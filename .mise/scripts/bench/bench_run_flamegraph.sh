...
# ── Combine flamegraphs into one CPU-scaling overview ────────────────────────
COMBINED_HTML=""
if [ "${SKIP_FLAMEGRAPH}" != "1" ]; then
  # Collect available per-worker SVGs (in worker-count order)
  COMBINE_SVG_ARGS=()
  for workers in ${WORKER_COUNTS}; do
    f="${FLAMEGRAPH_DIR}/ecdsa_sign_w${workers}.svg"
    if [ -f "${f}" ]; then
      COMBINE_SVG_ARGS+=("${f}")
    fi
  done

  if [ "${#COMBINE_SVG_ARGS[@]}" -gt 0 ]; then
    COMBINED_HTML="${FLAMEGRAPH_DIR}/cpu_scaling_combined.html"
    # Write a TSV of throughput data so the chart can be populated
    THROUGHPUT_TSV="${FLAMEGRAPH_DIR}/throughput.tsv"
    if [ -f "${THROUGHPUT_OUTPUT}" ]; then
      awk -v conc=16 '
        /^test .* bench:/ {
          name = $0
      ...
      ' "${THROUGHPUT_OUTPUT}" >"${THROUGHPUT_TSV}" 2>/dev/null || true
    fi

    COMBINE_ARGS=("${COMBINED_HTML}")
    if [ -f "${THROUGHPUT_TSV}" ]; then
      COMBINE_ARGS+=("${THROUGHPUT_TSV}")
    fi
    COMBINE_ARGS+=("${COMBINE_SVG_ARGS[@]}")

    python3 "${SCRIPT_DIR}/combine_flamegraphs.py" "${COMBINE_ARGS[@]}" \
      || echo "WARNING: combine_flamegraphs.py failed (non-fatal)"
  fi
fi
...
if [ "${SKIP_FLAMEGRAPH}" != "1" ]; then
  ...
  if [ -n "${COMBINED_HTML}" ] && [ -f "${COMBINED_HTML}" ]; then
    FLAMEGRAPH_SECTION+="### Combined scaling overview

[cpu_scaling_combined.html](${COMBINED_HTML##*/}) — all worker counts side-by-side with interactive flamegraphs.
...
echo "Written: ${OUT_MD}"
if [ -n "${COMBINED_HTML}" ] && [ -f "${COMBINED_HTML}" ]; then
  echo "Combined overview: ${COMBINED_HTML}"
fi
echo ""
