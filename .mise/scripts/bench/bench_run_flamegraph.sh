...
#   target/flamegraphs/ecdsa_w<N>.png               (PNG conversion of each SVG)
  ...
  for workers in ${WORKER_COUNTS}; do
    ...
    if [ -f "${SVG_OUT}" ]; then
      ...
      if command -v rsvg-convert >/dev/null 2>&1; then
        rsvg-convert --width 2400 "${SVG_OUT}" -o "${PNG_OUT}" \
          && echo "    Written: ${PNG_OUT}" \
          || echo "    WARNING: SVG→PNG conversion failed for ${SVG_OUT}"
      elif command -v convert >/dev/null 2>&1; then
        convert -density 150 "${SVG_OUT}" "${PNG_OUT}" \
          && echo "    Written: ${PNG_OUT}" \
          || echo "    WARNING: SVG→PNG conversion failed for ${SVG_OUT}"
      else
        echo "    WARNING: no SVG→PNG converter found (install librsvg2-bin or imagemagick)"
      fi
...
if [ "${SKIP_FLAMEGRAPH}" != "1" ]; then
  ...
  for workers in ${WORKER_COUNTS}; do
    PNG_OUT="${FLAMEGRAPH_DIR}/ecdsa_sign_w${workers}.png"
    SVG_OUT="${FLAMEGRAPH_DIR}/ecdsa_sign_w${workers}.svg"
    # Prefer PNG for the markdown report; fall back to SVG
    if [ -f "${PNG_OUT}" ]; then
      IMG_FILE="${PNG_OUT##*/}"
    elif [ -f "${SVG_OUT}" ]; then
      IMG_FILE="${SVG_OUT##*/}"
...
![flamegraph ${workers} workers](${IMG_FILE})
