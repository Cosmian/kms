...
# The PKCS#11 provider reaches for ckms.toml at runtime; the dataserver does not
# inherit the `CKMS_CONF` variable from an `isql` invocation, so it must be set
# before `startserver` launches the SAP ASE process.
...
# The dataserver loads PKCS#11 modules after startup, so its dynamic loader
# must inherit the ASE library directory before startserver forks it.
export LD_LIBRARY_PATH="${ASE_HOME}/ASE-16_1/lib${LD_LIBRARY_PATH:+:${LD_LIBRARY_PATH}}"
